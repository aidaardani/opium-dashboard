/* eslint-disable */
/* tslint:disable */
// @ts-nocheck
/* prettier-ignore-start */

/** @jsxRuntime classic */
/** @jsx createPlasmicElementProxy */
/** @jsxFrag React.Fragment */

// This class is auto-generated by Plasmic; please do not edit!
// Plasmic Project: 9g1e5LLLDS4TGJiaFCSEyH
// Component: E1ah_bVnKUPF

import * as React from "react";

import Head from "next/head";
import Link, { LinkProps } from "next/link";
import { useRouter } from "next/router";

import {
  Flex as Flex__,
  MultiChoiceArg,
  PlasmicDataSourceContextProvider as PlasmicDataSourceContextProvider__,
  PlasmicIcon as PlasmicIcon__,
  PlasmicImg as PlasmicImg__,
  PlasmicLink as PlasmicLink__,
  PlasmicPageGuard as PlasmicPageGuard__,
  SingleBooleanChoiceArg,
  SingleChoiceArg,
  Stack as Stack__,
  StrictProps,
  Trans as Trans__,
  classNames,
  createPlasmicElementProxy,
  deriveRenderOpts,
  ensureGlobalVariants,
  generateOnMutateForSpec,
  generateStateOnChangeProp,
  generateStateOnChangePropForCodeComponents,
  generateStateValueProp,
  get as $stateGet,
  hasVariant,
  initializeCodeComponentStates,
  initializePlasmicStates,
  makeFragment,
  omit,
  pick,
  renderPlasmicSlot,
  set as $stateSet,
  useCurrentUser,
  useDollarState,
  usePlasmicTranslator,
  useTrigger,
  wrapWithClassName
} from "@plasmicapp/react-web";
import {
  DataCtxReader as DataCtxReader__,
  useDataEnv,
  useGlobalActions
} from "@plasmicapp/react-web/lib/host";

import { ApiRequest } from "@/fragment/components/api-request"; // plasmic-import: Gl72hv5IMo-p/codeComponent
import ProfileExpertiseItem from "../../ProfileExpertiseItem"; // plasmic-import: E4hp1BPnaz1R/component
import Button from "../../Button"; // plasmic-import: oVzoHzMf1TLl/component
import Dialog from "../../Dialog"; // plasmic-import: FJiI2-N1is_F/component
import { _useGlobalVariants } from "./plasmic"; // plasmic-import: 9g1e5LLLDS4TGJiaFCSEyH/projectModule
import { _useStyleTokens } from "./PlasmicStyleTokensProvider"; // plasmic-import: 9g1e5LLLDS4TGJiaFCSEyH/styleTokensProvider

import "@plasmicapp/react-web/lib/plasmic.css";

import projectcss from "./plasmic.module.css"; // plasmic-import: 9g1e5LLLDS4TGJiaFCSEyH/projectcss
import sty from "./PlasmicProfileExperties.module.css"; // plasmic-import: E1ah_bVnKUPF/css

import Icon34Icon from "./icons/PlasmicIcon__Icon34"; // plasmic-import: Pu6FdA6kdBUA/icon
import ChevronRightIcon from "../fragment_icons/icons/PlasmicIcon__ChevronRight"; // plasmic-import: GHdF3hS-oP_3/icon
import ChevronLeftIcon from "../fragment_icons/icons/PlasmicIcon__ChevronLeft"; // plasmic-import: r9Upp9NbiZkf/icon
import Icon2Icon from "./icons/PlasmicIcon__Icon2"; // plasmic-import: q8mRvXMvOrv9/icon

import __lib_axios from "axios";
import { v4 as __lib_uuid__v4 } from "uuid";

createPlasmicElementProxy;

export type PlasmicProfileExperties__VariantMembers = {};
export type PlasmicProfileExperties__VariantsArgs = {};
type VariantPropType = keyof PlasmicProfileExperties__VariantsArgs;
export const PlasmicProfileExperties__VariantProps =
  new Array<VariantPropType>();

export type PlasmicProfileExperties__ArgsType = {
  degree?: any;
  expertise?: any;
};
type ArgPropType = keyof PlasmicProfileExperties__ArgsType;
export const PlasmicProfileExperties__ArgProps = new Array<ArgPropType>(
  "degree",
  "expertise"
);

export type PlasmicProfileExperties__OverridesType = {
  root?: Flex__<"div">;
  authApi?: Flex__<typeof ApiRequest>;
  expertisesApi?: Flex__<typeof ApiRequest>;
  profileExpertiseItem?: Flex__<typeof ProfileExpertiseItem>;
  h6?: Flex__<"h6">;
  profileExpertiseItem2?: Flex__<typeof ProfileExpertiseItem>;
  dialog?: Flex__<typeof Dialog>;
};

export interface DefaultProfileExpertiesProps {
  degree?: any;
  expertise?: any;
  className?: string;
}

const $$ = {
  axios: __lib_axios,
  uuid: {
    v4: __lib_uuid__v4
  }
};

function useNextRouter() {
  try {
    return useRouter();
  } catch {}
  return undefined;
}

function PlasmicProfileExperties__RenderFunc(props: {
  variants: PlasmicProfileExperties__VariantsArgs;
  args: PlasmicProfileExperties__ArgsType;
  overrides: PlasmicProfileExperties__OverridesType;
  forNode?: string;
}) {
  const { variants, overrides, forNode } = props;

  const args = React.useMemo(
    () =>
      Object.assign(
        {
          degree: [
            { id: "1", name: "\u06a9\u0627\u0631\u062f\u0627\u0646" },
            { id: "2", name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633" },
            {
              id: "3",
              name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f"
            },
            {
              id: "4",
              name: "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc"
            },
            { id: "5", name: "\u062f\u06a9\u062a\u0631\u0627\u06cc" },
            {
              id: "6",
              name: "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635"
            },
            { id: "7", name: "\u0645\u062a\u062e\u0635\u0635" },
            {
              id: "8",
              name: "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc"
            },
            {
              id: "9",
              name: "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc"
            },
            {
              id: "10",
              name: "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635"
            },
            { id: "11", name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635" },
            {
              id: "12",
              name: "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e"
            },
            { id: "13", name: "\u0641\u0644\u0648\u0634\u06cc\u067e" },
            { id: "14", name: "\u0646\u0627\u0645\u0634\u062e\u0635" },
            { id: "15", name: "\u0645\u0634\u0627\u0648\u0631" }
          ],
          expertise: [
            {
              code: "101103",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "1",
              name: "\u0627\u062a\u0627\u0642 \u0639\u0645\u0644"
            },
            {
              code: "101155",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "2",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u062d\u0631\u0641\u0647 \u0627\u06cc"
            },
            {
              code: "101156",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "3",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u062f\u0647\u0627\u0646"
            },
            {
              code: "101158",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "4",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "101160",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "5",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062d\u06cc\u0637"
            },
            {
              code: "101161",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "6",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062f\u0627\u0631\u0633"
            },
            {
              code: "101163",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "7",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a\u06a9\u0627\u0631 \u062f\u0647\u0627\u0646 \u0648 \u062f\u0646\u062f\u0627\u0646"
            },
            {
              code: "101192",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "8",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "101196",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "9",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "101204",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "10",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "101231",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "11",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0631\u062a\u0648\u062f\u0631\u0645\u0627\u0646\u06cc- \u0631\u0627\u062f\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc"
            },
            {
              code: "101232",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "12",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0631\u062a\u0648\u0634\u0646\u0627\u0633\u06cc- \u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "101233",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "13",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "101324",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "14",
              name: "\u0635\u0646\u0627\u06cc\u0639 \u0627\u06cc\u0645\u0646\u06cc \u0635\u0646\u0639\u062a\u06cc"
            },
            {
              code: "101340",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "15",
              name: "\u0639\u0644\u0648\u0645 \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc"
            },
            {
              code: "101358",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "16",
              name: "\u0641\u0648\u0631\u06cc\u062a\u0647\u0627\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "101381",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "17",
              name: "\u0645\u0627\u0645\u0627\u06cc\u06cc"
            },
            {
              code: "101382",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "18",
              name: "\u0645\u062f\u0627\u0631\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "101422",
              degree_id: "1",
              degree_name: "\u06a9\u0627\u0631\u062f\u0627\u0646",
              expertise_id: "19",
              name: "\u0647\u0648\u0634\u0628\u0631\u06cc"
            },
            {
              code: "111103",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "1",
              name: "\u0627\u062a\u0627\u0642 \u0639\u0645\u0644"
            },
            {
              code: "111117",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "20",
              name: "\u0628\u06cc\u0646\u0627\u06cc\u06cc \u0633\u0646\u062c\u06cc"
            },
            {
              code: "111120",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "21",
              name: "\u0627\u0639\u0636\u0627\u06cc \u0645\u0635\u0646\u0648\u0639\u06cc \u0648 \u0648\u0633\u0627\u06cc\u0644 \u06a9\u0645\u06a9\u06cc"
            },
            {
              code: "111158",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "4",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "111192",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "8",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "111229",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "22",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0631\u062a\u0648\u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "111232",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "12",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0631\u062a\u0648\u0634\u0646\u0627\u0633\u06cc- \u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "111233",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "13",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "111262",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "23",
              name: "\u062d\u0634\u0631\u0647 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0648 \u0645\u0628\u0627\u0631\u0632\u0647 \u0628\u0627 \u0646\u0627\u0642\u0644\u06cc\u0646"
            },
            {
              code: "111266",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "24",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc"
            },
            {
              code: "111285",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "25",
              name: "\u0631\u0634\u062f \u0648 \u067e\u0631\u0648\u0631\u0634 \u06a9\u0648\u062f\u06a9\u0627\u0646 \u067e\u06cc\u0634 \u062f\u0628\u0633\u062a\u0627\u0646\u06cc"
            },
            {
              code: "111292",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "26",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "111293",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "27",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "111307",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "28",
              name: "\u0633\u0627\u062e\u062a \u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "111321",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "29",
              name: "\u0634\u0646\u0648\u0627\u06cc\u06cc \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "111338",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "30",
              name: "\u0639\u0644\u0648\u0645 \u0627\u062c\u062a\u0645\u0627\u0639\u06cc- \u062e\u062f\u0645\u0627\u062a \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "111340",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "15",
              name: "\u0639\u0644\u0648\u0645 \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc"
            },
            {
              code: "111343",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "31",
              name: "\u0639\u0644\u0648\u0645 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "111348",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "32",
              name: "\u0639\u0644\u0648\u0645 \u0648 \u0635\u0646\u0627\u06cc\u0639 \u063a\u0630\u0627\u06cc\u06cc- \u06a9\u0646\u062a\u0631\u0644 \u06a9\u06cc\u0641\u06cc \u0648 \u0628\u0647\u062f\u0627\u0634\u062a\u06cc"
            },
            {
              code: "111355",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "33",
              name: "\u0641\u0646\u0627\u0648\u0631\u06cc \u0627\u0637\u0644\u0627\u0639\u0627\u062a \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "111358",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "16",
              name: "\u0641\u0648\u0631\u06cc\u062a\u0647\u0627\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "111360",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "34",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc"
            },
            {
              code: "111367",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "35",
              name: "\u06a9\u0627\u0631\u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "111369",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "36",
              name: "\u06a9\u062a\u0627\u0628\u062f\u0627\u0631\u06cc \u062f\u0631 \u0634\u0627\u062e\u0647 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "111374",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "37",
              name: "\u06af\u0641\u062a\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "111381",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "17",
              name: "\u0645\u0627\u0645\u0627\u06cc\u06cc"
            },
            {
              code: "111382",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "18",
              name: "\u0645\u062f\u0627\u0631\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "111383",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "38",
              name: "\u0645\u062f\u062f\u06a9\u0627\u0631\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "111390",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "39",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062e\u062f\u0645\u0627\u062a \u0628\u0647\u062f\u0627\u0634\u062a\u06cc \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "111394",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "40",
              name: "\u0645\u0634\u0627\u0648\u0631\u0647"
            },
            {
              code: "111401",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "41",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u062d\u0631\u0641\u0647 \u0627\u06cc"
            },
            {
              code: "111402",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "42",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062d\u06cc\u0637"
            },
            {
              code: "111405",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "43",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u0627\u0644\u06a9\u062a\u0631\u06cc\u06a9"
            },
            {
              code: "111407",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "44",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "111408",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "45",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0635\u0646\u0627\u06cc\u0639- \u0627\u06cc\u0645\u0646\u06cc \u0635\u0646\u0639\u062a\u06cc"
            },
            {
              code: "111422",
              degree_id: "2",
              degree_name: "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633",
              expertise_id: "19",
              name: "\u0647\u0648\u0634\u0628\u0631\u06cc"
            },
            {
              code: "141102",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "46",
              name: "\u0627\u067e\u06cc\u062f\u0645\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "141103",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "1",
              name: "\u0627\u062a\u0627\u0642 \u0639\u0645\u0644"
            },
            {
              code: "141106",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "47",
              name: "\u0627\u062e\u0644\u0627\u0642 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141111",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "48",
              name: "\u0627\u0631\u0632\u06cc\u0627\u0628\u06cc \u0641\u0646\u0627\u0648\u0631\u06cc \u0633\u0644\u0627\u0645\u062a- H.T.A"
            },
            {
              code: "141112",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "49",
              name: "\u0627\u0631\u06af\u0648\u0646\u0648\u0645\u06cc"
            },
            {
              code: "141119",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "50",
              name: "\u0627\u0639\u0636\u0627\u0621 \u0645\u0635\u0646\u0648\u0639\u06cc"
            },
            {
              code: "141121",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "51",
              name: "\u0627\u0642\u062a\u0635\u0627\u062f \u0628\u0647\u062f\u0627\u0634\u062a"
            },
            {
              code: "141128",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "52",
              name: "\u0627\u06a9\u0648\u0644\u0648\u0698\u06cc \u0627\u0646\u0633\u0627\u0646\u06cc"
            },
            {
              code: "141130",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "53",
              name: "\u0627\u0646\u0641\u0648\u0631\u0645\u0627\u062a\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141133",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "54",
              name: "\u0627\u0646\u06af\u0644 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141135",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "55",
              name: "\u0627\u06cc\u0645\u0646\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141143",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "56",
              name: "\u0622\u0645\u0627\u0631 \u0632\u06cc\u0633\u062a\u06cc"
            },
            {
              code: "141144",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "57",
              name: "\u0622\u0645\u0648\u0632\u0634 \u0628\u0647\u062f\u0627\u0634\u062a"
            },
            {
              code: "141146",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "58",
              name: "\u0622\u0645\u0648\u0632\u0634 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141147",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "59",
              name: "\u0622\u0645\u0648\u0632\u0634 \u062c\u0627\u0645\u0639\u0647 \u0646\u06af\u0631 \u062f\u0631 \u0646\u0638\u0627\u0645 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "141149",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "60",
              name: "\u0628\u0627\u0641\u062a \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "141151",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "61",
              name: "\u0628\u0627\u06a9\u062a\u0631\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141152",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "62",
              name: "\u0628\u0631\u0646\u0627\u0645\u0647\u200c\u0631\u06cc\u0632\u06cc \u06cc\u0627\u062f\u06af\u06cc\u0631\u06cc \u0627\u0644\u06a9\u062a\u0631\u0648\u0646\u06cc\u06a9\u06cc \u062f\u0631 \u0639\u0644\u0648\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141154",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "63",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u067e\u0631\u062a\u0648\u0647\u0627"
            },
            {
              code: "141157",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "64",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0631\u0648\u0627\u0646"
            },
            {
              code: "141162",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "65",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0648 \u0627\u06cc\u0645\u0646\u06cc \u0645\u0648\u0627\u062f \u063a\u0630\u0627\u06cc\u06cc"
            },
            {
              code: "141177",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "20",
              name: "\u0628\u06cc\u0646\u0627\u06cc\u06cc \u0633\u0646\u062c\u06cc"
            },
            {
              code: "141178",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "66",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc"
            },
            {
              code: "141184",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "67",
              name: "\u0628\u06cc\u0648\u0634\u06cc\u0645\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "141192",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "8",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "141193",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "68",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0627\u0648\u0631\u0698\u0627\u0646\u0633"
            },
            {
              code: "141194",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "69",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "141195",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "70",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u062f\u0627\u062e\u0644\u06cc- \u062c\u0631\u0627\u062d\u06cc"
            },
            {
              code: "141197",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "71",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0633\u0627\u0644\u0645\u0646\u062f\u06cc"
            },
            {
              code: "141198",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "72",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0633\u0644\u0627\u0645\u062a \u062c\u0627\u0645\u0639\u0647"
            },
            {
              code: "141199",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "73",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "141200",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "74",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647"
            },
            {
              code: "141201",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "75",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647 \u0646\u0648\u0632\u0627\u062f\u0627\u0646"
            },
            {
              code: "141202",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "76",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc \u0646\u0638\u0627\u0645\u06cc"
            },
            {
              code: "141217",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "77",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0648\u0631\u0632\u0634\u06cc"
            },
            {
              code: "141219",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "78",
              name: "\u067e\u06cc\u0634\u06af\u06cc\u0631\u06cc \u0627\u0632 \u0622\u0633\u06cc\u0628 \u0647\u0627\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "141224",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "79",
              name: "\u062a\u0627\u0631\u06cc\u062e \u0639\u0644\u0648\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141225",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "80",
              name: "\u062a\u0631\u06a9\u06cc\u0628\u0627\u062a \u0637\u0628\u06cc\u0639\u06cc \u0648 \u062f\u0627\u0631\u0648\u06cc\u06cc \u062f\u0631\u06cc\u0627\u06cc\u06cc"
            },
            {
              code: "141235",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "81",
              name: "\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u06af\u0631\u062f\u0634 \u062e\u0648\u0646"
            },
            {
              code: "141262",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "23",
              name: "\u062d\u0634\u0631\u0647 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0648 \u0645\u0628\u0627\u0631\u0632\u0647 \u0628\u0627 \u0646\u0627\u0642\u0644\u06cc\u0646"
            },
            {
              code: "141263",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "82",
              name: "\u062e\u0648\u0646 \u0634\u0646\u0627\u0633\u06cc \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc \u0648 \u0628\u0627\u0646\u06a9 \u062e\u0648\u0646"
            },
            {
              code: "141280",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "83",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0628\u06cc\u0648\u0644\u0648\u0698\u06cc \u0648 \u062d\u0641\u0627\u0638\u062a \u067e\u0631\u062a\u0648\u06cc\u06cc"
            },
            {
              code: "141286",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "84",
              name: "\u0631\u0641\u0627\u0647 \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "141287",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "85",
              name: "\u0631\u0648\u0627\u0646 \u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "141292",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "26",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "141293",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "27",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "141295",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "86",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0648\u0622\u0645\u0648\u0632\u0634 \u06a9\u0648\u062f\u06a9\u0627\u0646 \u0627\u0633\u062a\u062b\u0646\u0627\u06cc\u06cc"
            },
            {
              code: "141301",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "87",
              name: "\u0632\u06cc\u0633\u062a \u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141305",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "88",
              name: "\u0698\u0646\u062a\u06cc\u06a9 \u0627\u0646\u0633\u0627\u0646\u06cc"
            },
            {
              code: "141312",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "89",
              name: "\u0633\u0644\u0627\u0645\u062a \u0633\u0627\u0644\u0645\u0646\u062f\u06cc"
            },
            {
              code: "141313",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "90",
              name: "\u0633\u0644\u0627\u0645\u062a \u0648 \u0631\u0633\u0627\u0646\u0647"
            },
            {
              code: "141315",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "91",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "141317",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "92",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc \u0645\u062d\u06cc\u0637"
            },
            {
              code: "141321",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "29",
              name: "\u0634\u0646\u0648\u0627\u06cc\u06cc \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "141322",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "93",
              name: "\u0634\u06cc\u0645\u06cc \u062f\u0627\u0631\u0648\u0626\u06cc"
            },
            {
              code: "141330",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "94",
              name: "\u0637\u0628 \u0633\u0646\u062a\u06cc \u0627\u06cc\u0631\u0627\u0646\u06cc"
            },
            {
              code: "141341",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "95",
              name: "\u0639\u0644\u0648\u0645 \u0628\u0647\u062f\u0627\u0634\u062a\u06cc \u062f\u0631 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "141342",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "96",
              name: "\u0639\u0644\u0648\u0645 \u062a\u0634\u0631\u06cc\u062d\u06cc"
            },
            {
              code: "141343",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "31",
              name: "\u0639\u0644\u0648\u0645 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "141344",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "97",
              name: "\u0639\u0644\u0648\u0645 \u062a\u063a\u0630\u06cc\u0647 \u062f\u0631 \u0628\u062d\u0631\u0627\u0646 \u0648 \u062d\u0648\u0627\u062f\u062b \u063a\u06cc\u0631\u0645\u062a\u0631\u0642\u0628\u0647"
            },
            {
              code: "141345",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "98",
              name: "\u0639\u0644\u0648\u0645 \u062f\u0627\u0631\u0648\u0647\u0627\u06cc \u067e\u0631\u062a\u0648\u0632\u0627"
            },
            {
              code: "141347",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "99",
              name: "\u0639\u0644\u0648\u0645 \u06a9\u062a\u0627\u0628\u062f\u0627\u0631\u06cc \u0648 \u0627\u0637\u0644\u0627\u0639 \u0631\u0633\u0627\u0646\u06cc"
            },
            {
              code: "141348",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "32",
              name: "\u0639\u0644\u0648\u0645 \u0648 \u0635\u0646\u0627\u06cc\u0639 \u063a\u0630\u0627\u06cc\u06cc- \u06a9\u0646\u062a\u0631\u0644 \u06a9\u06cc\u0641\u06cc \u0648 \u0628\u0647\u062f\u0627\u0634\u062a\u06cc"
            },
            {
              code: "141355",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "33",
              name: "\u0641\u0646\u0627\u0648\u0631\u06cc \u0627\u0637\u0644\u0627\u0639\u0627\u062a \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "141356",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "100",
              name: "\u0641\u0646\u0627\u0648\u0631\u06cc \u062a\u0635\u0648\u06cc\u0631\u0628\u0631\u062f\u0627\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc- MRI"
            },
            {
              code: "141357",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "101",
              name: "\u0641\u0646\u0627\u0648\u0631\u06cc \u062a\u0635\u0648\u06cc\u0631\u0628\u0631\u062f\u0627\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0633\u06cc\u062a\u06cc \u0627\u0633\u06a9\u0646"
            },
            {
              code: "141359",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "102",
              name: "\u0641\u06cc\u0632\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141360",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "34",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc"
            },
            {
              code: "141361",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "103",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc \u0648\u0631\u0632\u0634\u06cc"
            },
            {
              code: "141362",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "104",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "141364",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "105",
              name: "\u0642\u0627\u0631\u0686 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141367",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "106",
              name: "\u06a9\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "141368",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "107",
              name: "\u06a9\u0627\u0631\u0622\u0641\u0631\u06cc\u0646\u06cc \u062f\u0631 \u0646\u0638\u0627\u0645 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "141372",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "108",
              name: "\u06a9\u0646\u062a\u0631\u0644 \u0645\u0648\u0627\u062f \u062e\u0648\u0631\u0627\u06a9\u06cc \u0648 \u0627\u0634\u0627\u0645\u06cc\u062f\u0646\u06cc"
            },
            {
              code: "141374",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "109",
              name: "\u06af\u0641\u062a\u0627\u0631\u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "141381",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "17",
              name: "\u0645\u0627\u0645\u0627\u06cc\u06cc"
            },
            {
              code: "141382",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "18",
              name: "\u0645\u062f\u0627\u0631\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141383",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "38",
              name: "\u0645\u062f\u062f\u06a9\u0627\u0631\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "141384",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "110",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a MBA"
            },
            {
              code: "141387",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "111",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u067e\u0633\u0645\u0627\u0646\u062f"
            },
            {
              code: "141389",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "112",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "141390",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "39",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062e\u062f\u0645\u0627\u062a \u0628\u0647\u062f\u0627\u0634\u062a\u06cc \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "141391",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "113",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u0633\u0644\u0627\u0645\u062a\u060c \u0627\u06cc\u0645\u0646\u06cc \u0648 \u0645\u062d\u06cc\u0637 \u0632\u06cc\u0633\u062a"
            },
            {
              code: "141394",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "40",
              name: "\u0645\u0634\u0627\u0648\u0631\u0647"
            },
            {
              code: "141396",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "114",
              name: "\u0645\u0634\u0627\u0648\u0631\u0647 \u062f\u0631 \u0645\u0627\u0645\u0627\u06cc\u06cc"
            },
            {
              code: "141399",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "115",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0627\u06cc\u0645\u0646\u06cc \u0635\u0646\u0639\u062a\u06cc"
            },
            {
              code: "141401",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "41",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u062d\u0631\u0641\u0647 \u0627\u06cc"
            },
            {
              code: "141403",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "116",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646"
            },
            {
              code: "141405",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "43",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u0627\u0644\u06a9\u062a\u0631\u06cc\u06a9"
            },
            {
              code: "141406",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "117",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0632\u06cc\u0633\u062a \u0645\u0648\u0627\u062f"
            },
            {
              code: "141411",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "118",
              name: "\u0645\u06cc\u06a9\u0631\u0648\u0628\u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "141412",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "119",
              name: "\u0645\u06cc\u06a9\u0631\u0648\u0628\u0634\u0646\u0627\u0633\u06cc \u0645\u0648\u0627\u062f \u063a\u0630\u0627\u06cc\u06cc"
            },
            {
              code: "141417",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "120",
              name: "\u0646\u0627\u0646\u0648\u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141418",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "121",
              name: "\u0646\u0638\u0627\u0631\u062a \u0628\u0631 \u0627\u0645\u0648\u0631 \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "141424",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "122",
              name: "\u0648\u06cc\u0631\u0648\u0633 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "141492",
              degree_id: "3",
              degree_name:
                "\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0627\u0631\u0634\u062f",
              expertise_id: "42",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062d\u06cc\u0637"
            },
            {
              code: "150212",
              degree_id: "4",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc",
              expertise_id: "123",
              name: "\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "151212",
              degree_id: "5",
              degree_name: "\u062f\u06a9\u062a\u0631\u0627\u06cc",
              expertise_id: "123",
              name: "\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "151266",
              degree_id: "5",
              degree_name: "\u062f\u06a9\u062a\u0631\u0627\u06cc",
              expertise_id: "24",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc"
            },
            {
              code: "151275",
              degree_id: "5",
              degree_name: "\u062f\u06a9\u062a\u0631\u0627\u06cc",
              expertise_id: "124",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "151340",
              degree_id: "5",
              degree_name: "\u062f\u06a9\u062a\u0631\u0627\u06cc",
              expertise_id: "15",
              name: "\u0639\u0644\u0648\u0645 \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc"
            },
            {
              code: "170102",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "46",
              name: "\u0627\u067e\u06cc\u062f\u0645\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "170108",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "125",
              name: "\u0627\u0631\u062a\u0648\u067e\u062f\u06cc"
            },
            {
              code: "170110",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "126",
              name: "\u0627\u0631\u062a\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "170129",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "127",
              name: "\u0627\u0646\u062f\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "170141",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "128",
              name: "\u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "170159",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "129",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0645\u0627\u062f\u0631 \u0648 \u06a9\u0648\u062f\u06a9"
            },
            {
              code: "170165",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "130",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u067e\u0648\u0633\u062a"
            },
            {
              code: "170166",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "131",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u062f\u0627\u062e\u0644\u06cc"
            },
            {
              code: "170168",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "132",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u062f\u0647\u0627\u0646 \u0648\u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "170172",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "133",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0639\u0641\u0648\u0646\u06cc \u0648 \u06af\u0631\u0645\u0633\u06cc\u0631\u06cc"
            },
            {
              code: "170174",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "134",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "170176",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "135",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "170178",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "66",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc"
            },
            {
              code: "170189",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "136",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9- \u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc \u062f\u0647\u0627\u0646 \u0648 \u062f\u0646\u062f\u0627\u0646"
            },
            {
              code: "170191",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "137",
              name: "\u067e\u0631\u062a\u0648\u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "170204",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "10",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "170211",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "138",
              name: "\u067e\u0631\u06cc\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "170213",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "139",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "170214",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "140",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0642\u0627\u0646\u0648\u0646\u06cc"
            },
            {
              code: "170216",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "141",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "170217",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "77",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0648\u0631\u0632\u0634\u06cc"
            },
            {
              code: "170245",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "142",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "170250",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "143",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "170255",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "144",
              name: "\u062c\u0631\u0627\u062d\u06cc \u06a9\u0644\u06cc\u0647 \u0648 \u0645\u062c\u0627\u0631\u06cc \u0627\u062f\u0631\u0627\u0631\u06cc \u062a\u0646\u0627\u0633\u0644\u06cc"
            },
            {
              code: "170257",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "145",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "170260",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "146",
              name: "\u0686\u0634\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "170267",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "147",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc- \u06a9\u0644\u06cc\u0646\u06cc\u06a9\u0627\u0644 \u0641\u0627\u0631\u0645\u0627\u0633\u06cc"
            },
            {
              code: "170278",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "148",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc"
            },
            {
              code: "170279",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "149",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "170281",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "150",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "170283",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "151",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "170288",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "152",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "170300",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "153",
              name: "\u0632\u0646\u0627\u0646 \u0648 \u0632\u0627\u06cc\u0645\u0627\u0646"
            },
            {
              code: "170325",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "154",
              name: "\u0637\u0628 \u0627\u0648\u0631\u0698\u0627\u0646\u0633"
            },
            {
              code: "170329",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "155",
              name: "\u0637\u0628 \u0633\u0627\u0644\u0645\u0646\u062f\u06cc"
            },
            {
              code: "170331",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "156",
              name: "\u0637\u0628 \u0641\u06cc\u0632\u06cc\u06a9\u06cc \u0648 \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "170332",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "157",
              name: "\u0637\u0628 \u06a9\u0627\u0631"
            },
            {
              code: "170335",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "158",
              name: "\u0637\u0628 \u0647\u0648\u0627 \u0648 \u0641\u0636\u0627 \u0648 \u0632\u06cc\u0631 \u0633\u0637\u062d\u06cc"
            },
            {
              code: "170373",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "159",
              name: "\u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "170378",
              degree_id: "6",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062a\u062e\u0635\u0635",
              expertise_id: "160",
              name: "\u06af\u0648\u0634 \u0648 \u062d\u0644\u0642 \u0648 \u0628\u06cc\u0646\u06cc \u0648 \u062c\u0631\u0627\u062d\u06cc \u0633\u0631 \u0648 \u06af\u0631\u062f\u0646"
            },
            {
              code: "171102",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "46",
              name: "\u0627\u067e\u06cc\u062f\u0645\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "171108",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "125",
              name: "\u0627\u0631\u062a\u0648\u067e\u062f\u06cc"
            },
            {
              code: "171110",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "126",
              name: "\u0627\u0631\u062a\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "171129",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "127",
              name: "\u0627\u0646\u062f\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "171141",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "128",
              name: "\u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "171159",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "129",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0645\u0627\u062f\u0631 \u0648 \u06a9\u0648\u062f\u06a9"
            },
            {
              code: "171165",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "130",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u067e\u0648\u0633\u062a"
            },
            {
              code: "171166",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "131",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u062f\u0627\u062e\u0644\u06cc"
            },
            {
              code: "171168",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "132",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u062f\u0647\u0627\u0646 \u0648\u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "171172",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "133",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0639\u0641\u0648\u0646\u06cc \u0648 \u06af\u0631\u0645\u0633\u06cc\u0631\u06cc"
            },
            {
              code: "171174",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "134",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "171176",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "135",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "171178",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "66",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc"
            },
            {
              code: "171189",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "136",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9- \u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc \u062f\u0647\u0627\u0646 \u0648 \u062f\u0646\u062f\u0627\u0646"
            },
            {
              code: "171191",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "137",
              name: "\u067e\u0631\u062a\u0648\u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "171204",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "10",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "171211",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "138",
              name: "\u067e\u0631\u06cc\u0648\u062f\u0627\u0646\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "171213",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "139",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "171214",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "140",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0642\u0627\u0646\u0648\u0646\u06cc"
            },
            {
              code: "171216",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "141",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "171217",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "77",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0648\u0631\u0632\u0634\u06cc"
            },
            {
              code: "171245",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "142",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "171250",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "143",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "171255",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "144",
              name: "\u062c\u0631\u0627\u062d\u06cc \u06a9\u0644\u06cc\u0647 \u0648 \u0645\u062c\u0627\u0631\u06cc \u0627\u062f\u0631\u0627\u0631\u06cc \u062a\u0646\u0627\u0633\u0644\u06cc"
            },
            {
              code: "171257",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "145",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "171260",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "146",
              name: "\u0686\u0634\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "171267",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "147",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc- \u06a9\u0644\u06cc\u0646\u06cc\u06a9\u0627\u0644 \u0641\u0627\u0631\u0645\u0627\u0633\u06cc"
            },
            {
              code: "171278",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "148",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc"
            },
            {
              code: "171279",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "149",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "171281",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "150",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "171283",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "151",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "171288",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "152",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "171300",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "153",
              name: "\u0632\u0646\u0627\u0646 \u0648 \u0632\u0627\u06cc\u0645\u0627\u0646"
            },
            {
              code: "171325",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "154",
              name: "\u0637\u0628 \u0627\u0648\u0631\u0698\u0627\u0646\u0633"
            },
            {
              code: "171329",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "155",
              name: "\u0637\u0628 \u0633\u0627\u0644\u0645\u0646\u062f\u06cc"
            },
            {
              code: "171331",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "156",
              name: "\u0637\u0628 \u0641\u06cc\u0632\u06cc\u06a9\u06cc \u0648 \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "171332",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "157",
              name: "\u0637\u0628 \u06a9\u0627\u0631"
            },
            {
              code: "171335",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "158",
              name: "\u0637\u0628 \u0647\u0648\u0627 \u0648 \u0641\u0636\u0627 \u0648 \u0632\u06cc\u0631 \u0633\u0637\u062d\u06cc"
            },
            {
              code: "171373",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "159",
              name: "\u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "171378",
              degree_id: "7",
              degree_name: "\u0645\u062a\u062e\u0635\u0635",
              expertise_id: "160",
              name: "\u06af\u0648\u0634 \u0648 \u062d\u0644\u0642 \u0648 \u0628\u06cc\u0646\u06cc \u0648 \u062c\u0631\u0627\u062d\u06cc \u0633\u0631 \u0648 \u06af\u0631\u062f\u0646"
            },
            {
              code: "190102",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "46",
              name: "\u0627\u067e\u06cc\u062f\u0645\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "190106",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "47",
              name: "\u0627\u062e\u0644\u0627\u0642 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190119",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "161",
              name: "\u0627\u0639\u0636\u0627\u06cc \u0645\u0635\u0646\u0648\u0639\u06cc"
            },
            {
              code: "190122",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "162",
              name: "\u0627\u0642\u062a\u0635\u0627\u062f \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "190123",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "163",
              name: "\u0627\u0642\u062a\u0635\u0627\u062f \u0648 \u0645\u062f\u06cc\u0631\u06cc\u062a \u062f\u0627\u0631\u0648"
            },
            {
              code: "190130",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "53",
              name: "\u0627\u0646\u0641\u0648\u0631\u0645\u0627\u062a\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190133",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "54",
              name: "\u0627\u0646\u06af\u0644 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190135",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "55",
              name: "\u0627\u06cc\u0645\u0646\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190143",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "56",
              name: "\u0622\u0645\u0627\u0631 \u0632\u06cc\u0633\u062a\u06cc"
            },
            {
              code: "190145",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "164",
              name: "\u0622\u0645\u0648\u0632\u0634 \u0628\u0647\u062f\u0627\u0634\u062a \u0648 \u0627\u0631\u062a\u0642\u0627\u0621 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "190146",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "58",
              name: "\u0622\u0645\u0648\u0632\u0634 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190150",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "165",
              name: "\u0628\u0627\u0641\u062a \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190151",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "61",
              name: "\u0628\u0627\u06a9\u062a\u0631\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190153",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "166",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0628\u0627\u0631\u0648\u0631\u06cc"
            },
            {
              code: "190177",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "20",
              name: "\u0628\u06cc\u0646\u0627\u06cc\u06cc \u0633\u0646\u062c\u06cc"
            },
            {
              code: "190184",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "67",
              name: "\u0628\u06cc\u0648\u0634\u06cc\u0645\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "190185",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "167",
              name: "\u0628\u06cc\u0648\u0644\u0648\u0698\u06cc \u062a\u0648\u0644\u06cc\u062f\u0645\u062b\u0644"
            },
            {
              code: "190192",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "8",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "190207",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "168",
              name: "\u067e\u0631\u0648\u062a\u0626\u0648\u0645\u06cc\u06a9\u0633 \u06a9\u0627\u0631\u0628\u0631\u062f\u06cc"
            },
            {
              code: "190215",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "169",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0645\u0648\u0644\u06a9\u0648\u0644\u06cc"
            },
            {
              code: "190224",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "79",
              name: "\u062a\u0627\u0631\u06cc\u062e \u0639\u0644\u0648\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190236",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "170",
              name: "\u062a\u0648\u06a9\u0633\u06cc\u0646\u200c\u0647\u0627\u06cc \u0645\u06cc\u06a9\u0631\u0648\u0628\u06cc"
            },
            {
              code: "190262",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "23",
              name: "\u062d\u0634\u0631\u0647 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0648 \u0645\u0628\u0627\u0631\u0632\u0647 \u0628\u0627 \u0646\u0627\u0642\u0644\u06cc\u0646"
            },
            {
              code: "190268",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "171",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0633\u0646\u062a\u06cc"
            },
            {
              code: "190291",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "172",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0646\u0638\u0627\u0645\u06cc"
            },
            {
              code: "190292",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "26",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "190302",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "173",
              name: "\u0632\u06cc\u0633\u062a \u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190303",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "174",
              name: "\u0632\u06cc\u0633\u062a \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc- \u0628\u06cc\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "190304",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "175",
              name: "\u0632\u06cc\u0633\u062a \u0645\u0648\u0627\u062f \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "190306",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "176",
              name: "\u0698\u0646\u062a\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190308",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "177",
              name: "\u0633\u0627\u0644\u0645\u0646\u062f \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "190310",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "178",
              name: "\u0633\u0644\u0627\u0645\u062a \u062f\u0631 \u0628\u0644\u0627\u06cc\u0627 \u0648 \u0641\u0648\u0631\u06cc\u062a \u0647\u0627"
            },
            {
              code: "190311",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "179",
              name: "\u0633\u0644\u0627\u0645\u062a \u062f\u0647\u0627\u0646 \u0648 \u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "190314",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "180",
              name: "\u0633\u0644\u0627\u0645\u062a \u0648 \u0631\u0641\u0627\u0647 \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "190315",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "91",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "190319",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "181",
              name: "\u0633\u06cc\u0627\u0633\u062a \u06af\u0630\u0627\u0631\u06cc \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "190320",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "182",
              name: "\u0633\u06cc\u0627\u0633\u062a\u0647\u0627\u06cc \u063a\u0630\u0627 \u0648 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "190321",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "29",
              name: "\u0634\u0646\u0648\u0627\u06cc\u06cc \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "190322",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "183",
              name: "\u0634\u06cc\u0645\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "190323",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "184",
              name: "\u0634\u06cc\u0645\u06cc \u0645\u0648\u0627\u062f \u062e\u0648\u0631\u0627\u06a9\u06cc \u0648 \u0622\u0628\u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190330",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "94",
              name: "\u0637\u0628 \u0633\u0646\u062a\u06cc \u0627\u06cc\u0631\u0627\u0646\u06cc"
            },
            {
              code: "190339",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "185",
              name: "\u0639\u0644\u0648\u0645 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "190342",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "96",
              name: "\u0639\u0644\u0648\u0645 \u062a\u0634\u0631\u06cc\u062d\u06cc"
            },
            {
              code: "190343",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "31",
              name: "\u0639\u0644\u0648\u0645 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "190346",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "186",
              name: "\u0639\u0644\u0648\u0645 \u0633\u0644\u0648\u0644\u06cc \u06a9\u0627\u0631\u0628\u0631\u062f\u06cc"
            },
            {
              code: "190348",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "32",
              name: "\u0639\u0644\u0648\u0645 \u0648 \u0635\u0646\u0627\u06cc\u0639 \u063a\u0630\u0627\u06cc\u06cc- \u06a9\u0646\u062a\u0631\u0644 \u06a9\u06cc\u0641\u06cc \u0648 \u0628\u0647\u062f\u0627\u0634\u062a\u06cc"
            },
            {
              code: "190352",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "187",
              name: "\u0641\u0627\u0631\u0645\u0627\u0633\u06cc\u0648\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "190353",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "188",
              name: "\u0641\u0627\u0631\u0645\u0627\u06a9\u0648\u06af\u0646\u0648\u0632\u06cc"
            },
            {
              code: "190354",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "189",
              name: "\u0641\u0631\u0622\u0648\u0631\u062f\u0647 \u0647\u0627\u06cc \u0628\u06cc\u0648\u0644\u0648\u0698\u06cc\u06a9"
            },
            {
              code: "190359",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "102",
              name: "\u0641\u06cc\u0632\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190360",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "34",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc"
            },
            {
              code: "190362",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "104",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "190363",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "190",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc \u0648\u0631\u0632\u0634"
            },
            {
              code: "190364",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "105",
              name: "\u0642\u0627\u0631\u0686 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190367",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "106",
              name: "\u06a9\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "190369",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "191",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "190371",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "192",
              name: "\u06a9\u0646\u062a\u0631\u0644 \u062f\u0627\u0631\u0648"
            },
            {
              code: "190374",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "37",
              name: "\u06af\u0641\u062a\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "190383",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "38",
              name: "\u0645\u062f\u062f\u06a9\u0627\u0631\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "190386",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "193",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u0627\u0637\u0644\u0627\u0639\u0627\u062a \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "190388",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "194",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062a\u062d\u0642\u06cc\u0642\u0627\u062a \u0648 \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0631 \u0646\u0638\u0627\u0645 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "190390",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "39",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062e\u062f\u0645\u0627\u062a \u0628\u0647\u062f\u0627\u0634\u062a\u06cc \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "190395",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "195",
              name: "\u0645\u0634\u0627\u0648\u0631\u0647 \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "190397",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "196",
              name: "\u0645\u0637\u0627\u0644\u0639\u0627\u062a \u0627\u0639\u062a\u06cc\u0627\u062f"
            },
            {
              code: "190400",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "197",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0627\u0641\u062a"
            },
            {
              code: "190401",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "41",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u062d\u0631\u0641\u0647 \u0627\u06cc"
            },
            {
              code: "190402",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "42",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062d\u06cc\u0637"
            },
            {
              code: "190404",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "198",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190405",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "43",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u0627\u0644\u06a9\u062a\u0631\u06cc\u06a9"
            },
            {
              code: "190409",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "199",
              name: "\u0645\u0648\u0627\u062f \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "190410",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "200",
              name: "\u0645\u06cc\u06a9\u0631\u0648\u0628 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190416",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "201",
              name: "\u0646\u0627\u0646\u0648 \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc- \u0646\u0627\u0646\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "190417",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "120",
              name: "\u0646\u0627\u0646\u0648\u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190421",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "202",
              name: "\u0647\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc \u0648 \u0628\u0627\u0646\u06a9 \u062e\u0648\u0646"
            },
            {
              code: "190424",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "122",
              name: "\u0648\u06cc\u0631\u0648\u0633 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "190429",
              degree_id: "8",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "203",
              name: "\u0641\u0627\u0631\u0645\u0627\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "191102",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "46",
              name: "\u0627\u067e\u06cc\u062f\u0645\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "191106",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "47",
              name: "\u0627\u062e\u0644\u0627\u0642 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191119",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "161",
              name: "\u0627\u0639\u0636\u0627\u06cc \u0645\u0635\u0646\u0648\u0639\u06cc"
            },
            {
              code: "191122",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "162",
              name: "\u0627\u0642\u062a\u0635\u0627\u062f \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "191123",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "163",
              name: "\u0627\u0642\u062a\u0635\u0627\u062f \u0648 \u0645\u062f\u06cc\u0631\u06cc\u062a \u062f\u0627\u0631\u0648"
            },
            {
              code: "191130",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "53",
              name: "\u0627\u0646\u0641\u0648\u0631\u0645\u0627\u062a\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191133",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "54",
              name: "\u0627\u0646\u06af\u0644 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191135",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "55",
              name: "\u0627\u06cc\u0645\u0646\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191143",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "56",
              name: "\u0622\u0645\u0627\u0631 \u0632\u06cc\u0633\u062a\u06cc"
            },
            {
              code: "191145",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "164",
              name: "\u0622\u0645\u0648\u0632\u0634 \u0628\u0647\u062f\u0627\u0634\u062a \u0648 \u0627\u0631\u062a\u0642\u0627\u0621 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "191146",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "58",
              name: "\u0622\u0645\u0648\u0632\u0634 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191150",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "165",
              name: "\u0628\u0627\u0641\u062a \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191151",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "61",
              name: "\u0628\u0627\u06a9\u062a\u0631\u06cc \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191153",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "166",
              name: "\u0628\u0647\u062f\u0627\u0634\u062a \u0628\u0627\u0631\u0648\u0631\u06cc"
            },
            {
              code: "191158",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "204",
              name: "\u062f\u06a9\u062a\u0631\u06cc \u062a\u062e\u0635\u0635\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "191177",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "20",
              name: "\u0628\u06cc\u0646\u0627\u06cc\u06cc \u0633\u0646\u062c\u06cc"
            },
            {
              code: "191184",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "67",
              name: "\u0628\u06cc\u0648\u0634\u06cc\u0645\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "191185",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "167",
              name: "\u0628\u06cc\u0648\u0644\u0648\u0698\u06cc \u062a\u0648\u0644\u06cc\u062f\u0645\u062b\u0644"
            },
            {
              code: "191192",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "8",
              name: "\u067e\u0631\u0633\u062a\u0627\u0631\u06cc"
            },
            {
              code: "191207",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "168",
              name: "\u067e\u0631\u0648\u062a\u0626\u0648\u0645\u06cc\u06a9\u0633 \u06a9\u0627\u0631\u0628\u0631\u062f\u06cc"
            },
            {
              code: "191215",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "169",
              name: "\u067e\u0632\u0634\u06a9\u06cc \u0645\u0648\u0644\u06a9\u0648\u0644\u06cc"
            },
            {
              code: "191224",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "79",
              name: "\u062a\u0627\u0631\u06cc\u062e \u0639\u0644\u0648\u0645 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191236",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "170",
              name: "\u062a\u0648\u06a9\u0633\u06cc\u0646\u200c\u0647\u0627\u06cc \u0645\u06cc\u06a9\u0631\u0648\u0628\u06cc"
            },
            {
              code: "191262",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "23",
              name: "\u062d\u0634\u0631\u0647 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc \u0648 \u0645\u0628\u0627\u0631\u0632\u0647 \u0628\u0627 \u0646\u0627\u0642\u0644\u06cc\u0646"
            },
            {
              code: "191268",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "171",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0633\u0646\u062a\u06cc"
            },
            {
              code: "191291",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "172",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0646\u0638\u0627\u0645\u06cc"
            },
            {
              code: "191292",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "26",
              name: "\u0631\u0648\u0627\u0646\u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "191302",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "173",
              name: "\u0632\u06cc\u0633\u062a \u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191303",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "174",
              name: "\u0632\u06cc\u0633\u062a \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc- \u0628\u06cc\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "191304",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "175",
              name: "\u0632\u06cc\u0633\u062a \u0645\u0648\u0627\u062f \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "191306",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "176",
              name: "\u0698\u0646\u062a\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191308",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "177",
              name: "\u0633\u0627\u0644\u0645\u0646\u062f \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "191310",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "178",
              name: "\u0633\u0644\u0627\u0645\u062a \u062f\u0631 \u0628\u0644\u0627\u06cc\u0627 \u0648 \u0641\u0648\u0631\u06cc\u062a \u0647\u0627"
            },
            {
              code: "191311",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "179",
              name: "\u0633\u0644\u0627\u0645\u062a \u062f\u0647\u0627\u0646 \u0648 \u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "191314",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "180",
              name: "\u0633\u0644\u0627\u0645\u062a \u0648 \u0631\u0641\u0627\u0647 \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "191315",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "91",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "191319",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "181",
              name: "\u0633\u06cc\u0627\u0633\u062a \u06af\u0630\u0627\u0631\u06cc \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "191320",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "182",
              name: "\u0633\u06cc\u0627\u0633\u062a\u0647\u0627\u06cc \u063a\u0630\u0627 \u0648 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "191321",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "29",
              name: "\u0634\u0646\u0648\u0627\u06cc\u06cc \u0634\u0646\u0627\u0633\u06cc"
            },
            {
              code: "191322",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "183",
              name: "\u0634\u06cc\u0645\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "191323",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "184",
              name: "\u0634\u06cc\u0645\u06cc \u0645\u0648\u0627\u062f \u062e\u0648\u0631\u0627\u06a9\u06cc \u0648 \u0622\u0628\u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191330",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "94",
              name: "\u0637\u0628 \u0633\u0646\u062a\u06cc \u0627\u06cc\u0631\u0627\u0646\u06cc"
            },
            {
              code: "191339",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "185",
              name: "\u0639\u0644\u0648\u0645 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "191342",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "96",
              name: "\u0639\u0644\u0648\u0645 \u062a\u0634\u0631\u06cc\u062d\u06cc"
            },
            {
              code: "191343",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "31",
              name: "\u0639\u0644\u0648\u0645 \u062a\u063a\u0630\u06cc\u0647"
            },
            {
              code: "191346",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "186",
              name: "\u0639\u0644\u0648\u0645 \u0633\u0644\u0648\u0644\u06cc \u06a9\u0627\u0631\u0628\u0631\u062f\u06cc"
            },
            {
              code: "191348",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "32",
              name: "\u0639\u0644\u0648\u0645 \u0648 \u0635\u0646\u0627\u06cc\u0639 \u063a\u0630\u0627\u06cc\u06cc- \u06a9\u0646\u062a\u0631\u0644 \u06a9\u06cc\u0641\u06cc \u0648 \u0628\u0647\u062f\u0627\u0634\u062a\u06cc"
            },
            {
              code: "191352",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "187",
              name: "\u0641\u0627\u0631\u0645\u0627\u0633\u06cc\u0648\u062a\u06cc\u06a9\u0633"
            },
            {
              code: "191353",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "188",
              name: "\u0641\u0627\u0631\u0645\u0627\u06a9\u0648\u06af\u0646\u0648\u0632\u06cc"
            },
            {
              code: "191354",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "189",
              name: "\u0641\u0631\u0622\u0648\u0631\u062f\u0647 \u0647\u0627\u06cc \u0628\u06cc\u0648\u0644\u0648\u0698\u06cc\u06a9"
            },
            {
              code: "191359",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "102",
              name: "\u0641\u06cc\u0632\u06cc\u06a9 \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191360",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "34",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u062a\u0631\u0627\u067e\u06cc"
            },
            {
              code: "191362",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "104",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "191363",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "190",
              name: "\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc \u0648\u0631\u0632\u0634"
            },
            {
              code: "191364",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "105",
              name: "\u0642\u0627\u0631\u0686 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191367",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "106",
              name: "\u06a9\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "191369",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "191",
              name: "\u062f\u0627\u0631\u0648\u0633\u0627\u0632\u06cc \u0647\u0633\u062a\u0647 \u0627\u06cc"
            },
            {
              code: "191371",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "192",
              name: "\u06a9\u0646\u062a\u0631\u0644 \u062f\u0627\u0631\u0648"
            },
            {
              code: "191374",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "37",
              name: "\u06af\u0641\u062a\u0627\u0631 \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "191383",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "38",
              name: "\u0645\u062f\u062f\u06a9\u0627\u0631\u06cc \u0627\u062c\u062a\u0645\u0627\u0639\u06cc"
            },
            {
              code: "191386",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "193",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u0627\u0637\u0644\u0627\u0639\u0627\u062a \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "191388",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "194",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062a\u062d\u0642\u06cc\u0642\u0627\u062a \u0648 \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0631 \u0646\u0638\u0627\u0645 \u0633\u0644\u0627\u0645\u062a"
            },
            {
              code: "191390",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "39",
              name: "\u0645\u062f\u06cc\u0631\u06cc\u062a \u062e\u062f\u0645\u0627\u062a \u0628\u0647\u062f\u0627\u0634\u062a\u06cc \u062f\u0631\u0645\u0627\u0646\u06cc"
            },
            {
              code: "191395",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "195",
              name: "\u0645\u0634\u0627\u0648\u0631\u0647 \u062a\u0648\u0627\u0646\u0628\u062e\u0634\u06cc"
            },
            {
              code: "191397",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "196",
              name: "\u0645\u0637\u0627\u0644\u0639\u0627\u062a \u0627\u0639\u062a\u06cc\u0627\u062f"
            },
            {
              code: "191400",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "197",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0627\u0641\u062a"
            },
            {
              code: "191401",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "41",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u062d\u0631\u0641\u0647 \u0627\u06cc"
            },
            {
              code: "191402",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "42",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u0628\u0647\u062f\u0627\u0634\u062a \u0645\u062d\u06cc\u0637"
            },
            {
              code: "191404",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "198",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191405",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "43",
              name: "\u0645\u0647\u0646\u062f\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc- \u0628\u06cc\u0648\u0627\u0644\u06a9\u062a\u0631\u06cc\u06a9"
            },
            {
              code: "191409",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "199",
              name: "\u0645\u0648\u0627\u062f \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "191410",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "200",
              name: "\u0645\u06cc\u06a9\u0631\u0648\u0628 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191416",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "201",
              name: "\u0646\u0627\u0646\u0648 \u0641\u0646\u0627\u0648\u0631\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc- \u0646\u0627\u0646\u0648\u062a\u06a9\u0646\u0648\u0644\u0648\u0698\u06cc \u062f\u0627\u0631\u0648\u06cc\u06cc"
            },
            {
              code: "191417",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "120",
              name: "\u0646\u0627\u0646\u0648\u0641\u0646\u0627\u0648\u0631\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191421",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "202",
              name: "\u0647\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0622\u0632\u0645\u0627\u06cc\u0634\u06af\u0627\u0647\u06cc \u0648 \u0628\u0627\u0646\u06a9 \u062e\u0648\u0646"
            },
            {
              code: "191424",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "122",
              name: "\u0648\u06cc\u0631\u0648\u0633 \u0634\u0646\u0627\u0633\u06cc \u067e\u0632\u0634\u06a9\u06cc"
            },
            {
              code: "191429",
              degree_id: "9",
              degree_name:
                "\u062f\u06a9\u062a\u0631\u0627\u06cc \u062a\u062e\u0635\u0635\u06cc",
              expertise_id: "203",
              name: "\u0641\u0627\u0631\u0645\u0627\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "200137",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "205",
              name: "\u0627\u06cc\u0645\u0648\u0646\u0648\u0644\u0648\u0698\u06cc \u0648 \u0622\u0644\u0631\u0698\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "200169",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "206",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0631\u06cc\u0647"
            },
            {
              code: "200174",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "134",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "200237",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "207",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200239",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "208",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u0644\u0627\u0633\u062a\u06cc\u06a9\u060c \u062a\u0631\u0645\u06cc\u0645\u06cc \u0648 \u0633\u0648\u062e\u062a\u06af\u06cc"
            },
            {
              code: "200249",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "209",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0631\u0648\u0642"
            },
            {
              code: "200253",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "210",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0642\u0641\u0633\u0647 \u0635\u062f\u0631\u06cc"
            },
            {
              code: "200254",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "211",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "200264",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "212",
              name: "\u062e\u0648\u0646 \u0648 \u0633\u0631\u0637\u0627\u0646 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200265",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "213",
              name: "\u062e\u0648\u0646 \u0648 \u0633\u0631\u0637\u0627\u0646 \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "200289",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "214",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200290",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "215",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9 \u0648 \u0646\u0648\u062c\u0648\u0627\u0646"
            },
            {
              code: "200296",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "216",
              name: "\u0631\u0648\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "200297",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "217",
              name: "\u0631\u0648\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "200299",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "218",
              name: "\u0631\u06cc\u0647 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "200334",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "219",
              name: "\u0637\u0628 \u0646\u0648\u0632\u0627\u062f\u06cc \u0648 \u067e\u06cc\u0631\u0627\u0645\u0648\u0646 \u062a\u0648\u0644\u062f"
            },
            {
              code: "200336",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "220",
              name: "\u0639\u0641\u0648\u0646\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200337",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "221",
              name: "\u0639\u0641\u0648\u0646\u06cc \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "200350",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "222",
              name: "\u063a\u062f\u062f \u062f\u0631\u0648\u0646 \u0631\u06cc\u0632 \u0648 \u0645\u062a\u0627\u0628\u0648\u0644\u06cc\u0633\u0645"
            },
            {
              code: "200351",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "223",
              name: "\u063a\u062f\u062f \u062f\u0631\u0648\u0646 \u0631\u06cc\u0632 \u0648 \u0645\u062a\u0627\u0628\u0648\u0644\u06cc\u0633\u0645 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "200366",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "224",
              name: "\u0642\u0644\u0628 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200370",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "225",
              name: "\u06a9\u0644\u06cc\u0647 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200376",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "226",
              name: "\u06af\u0648\u0627\u0631\u0634 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "200377",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "227",
              name: "\u06af\u0648\u0627\u0631\u0634 \u0648 \u06a9\u0628\u062f \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "200398",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "228",
              name: "\u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "200419",
              degree_id: "10",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "229",
              name: "\u0646\u0641\u0631\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "201137",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "205",
              name: "\u0627\u06cc\u0645\u0648\u0646\u0648\u0644\u0648\u0698\u06cc \u0648 \u0622\u0644\u0631\u0698\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "201169",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "206",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0631\u06cc\u0647"
            },
            {
              code: "201174",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "134",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "201237",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "207",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201239",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "208",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u0644\u0627\u0633\u062a\u06cc\u06a9\u060c \u062a\u0631\u0645\u06cc\u0645\u06cc \u0648 \u0633\u0648\u062e\u062a\u06af\u06cc"
            },
            {
              code: "201249",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "209",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0631\u0648\u0642"
            },
            {
              code: "201253",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "210",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0642\u0641\u0633\u0647 \u0635\u062f\u0631\u06cc"
            },
            {
              code: "201254",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "211",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642"
            },
            {
              code: "201264",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "212",
              name: "\u062e\u0648\u0646 \u0648 \u0633\u0631\u0637\u0627\u0646 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201265",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "213",
              name: "\u062e\u0648\u0646 \u0648 \u0633\u0631\u0637\u0627\u0646 \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "201289",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "214",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201290",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "215",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9 \u0648 \u0646\u0648\u062c\u0648\u0627\u0646"
            },
            {
              code: "201296",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "216",
              name: "\u0631\u0648\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "201297",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "217",
              name: "\u0631\u0648\u0645\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "201299",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "218",
              name: "\u0631\u06cc\u0647 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "201334",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "219",
              name: "\u0637\u0628 \u0646\u0648\u0632\u0627\u062f\u06cc \u0648 \u067e\u06cc\u0631\u0627\u0645\u0648\u0646 \u062a\u0648\u0644\u062f"
            },
            {
              code: "201336",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "220",
              name: "\u0639\u0641\u0648\u0646\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201337",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "221",
              name: "\u0639\u0641\u0648\u0646\u06cc \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "201350",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "222",
              name: "\u063a\u062f\u062f \u062f\u0631\u0648\u0646 \u0631\u06cc\u0632 \u0648 \u0645\u062a\u0627\u0628\u0648\u0644\u06cc\u0633\u0645"
            },
            {
              code: "201351",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "223",
              name: "\u063a\u062f\u062f \u062f\u0631\u0648\u0646 \u0631\u06cc\u0632 \u0648 \u0645\u062a\u0627\u0628\u0648\u0644\u06cc\u0633\u0645 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "201366",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "224",
              name: "\u0642\u0644\u0628 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201370",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "225",
              name: "\u06a9\u0644\u06cc\u0647 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201376",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "226",
              name: "\u06af\u0648\u0627\u0631\u0634 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "201377",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "227",
              name: "\u06af\u0648\u0627\u0631\u0634 \u0648 \u06a9\u0628\u062f \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "201398",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "228",
              name: "\u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "201419",
              degree_id: "11",
              degree_name: "\u0641\u0648\u0642 \u062a\u062e\u0635\u0635",
              expertise_id: "229",
              name: "\u0646\u0641\u0631\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210101",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "230",
              name: "HIV/AIDS  \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "210104",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "231",
              name: "\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0648 \u0646\u0648\u0631\u0648\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210105",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "232",
              name: "\u0627\u062e\u062a\u0644\u0627\u0644\u0627\u062a \u06a9\u0641 \u0644\u06af\u0646 \u062f\u0631 \u0632\u0646\u0627\u0646"
            },
            {
              code: "210107",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "233",
              name: "\u0627\u0631\u062a\u0648 \u0633\u0631\u062c\u0631\u06cc"
            },
            {
              code: "210109",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "234",
              name: "\u0627\u0631\u062a\u0648\u067e\u062f\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210113",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "235",
              name: "\u0627\u0631\u0648\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210114",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "236",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "210115",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "237",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc"
            },
            {
              code: "210116",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "238",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u0632\u0646\u0627\u0646 \u0648 \u0632\u0627\u06cc\u0645\u0627\u0646"
            },
            {
              code: "210117",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "239",
              name: "\u0627\u0633\u062a\u0631\u0627\u0628\u06cc\u0633\u0645"
            },
            {
              code: "210118",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "240",
              name: "\u0627\u0633\u062a\u0631\u06cc\u0648\u062a\u0627\u06a9\u0633\u06cc"
            },
            {
              code: "210124",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "241",
              name: "\u0627\u0642\u062f\u0627\u0645\u0627\u062a \u0645\u062f\u0627\u062e\u0644\u0647\u200c\u0627\u06cc \u0642\u0644\u0628 \u0628\u0632\u0631\u06af\u0633\u0627\u0644"
            },
            {
              code: "210125",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "242",
              name: "\u0627\u0642\u062f\u0627\u0645\u0627\u062a \u0645\u062f\u0627\u062e\u0644\u0647\u200c\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210126",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "243",
              name: "\u0627\u06a9\u0648 \u06a9\u0627\u0631\u062f\u06cc\u0648\u06af\u0631\u0627\u0641\u06cc"
            },
            {
              code: "210127",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "244",
              name: "\u0627\u06a9\u0648\u0644\u0648 \u067e\u0644\u0627\u0633\u062a\u06cc\u06a9"
            },
            {
              code: "210131",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "245",
              name: "\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc \u0632\u0646\u0627\u0646"
            },
            {
              code: "210132",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "246",
              name: "\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "210134",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "247",
              name: "\u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "210139",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "248",
              name: "\u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644 \u06a9\u0627\u0631\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210140",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "249",
              name: "\u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644 \u06a9\u0627\u0631\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0631\u06a9\u0648\u062f\u06a9\u0627\u0646 \u0648\u0646\u0648\u062c\u0648\u0627\u0646\u0627\u0646"
            },
            {
              code: "210142",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "250",
              name: "\u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc \u067e\u0648\u0633\u062a"
            },
            {
              code: "210148",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "251",
              name: "\u0622\u0646\u062f\u0648\u06cc\u0648\u0631\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210164",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "252",
              name: "\u0628\u0648\u06cc \u0628\u062f \u062f\u0647\u0627\u0646"
            },
            {
              code: "210170",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "253",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0633\u0637\u062d \u0686\u0634\u0645"
            },
            {
              code: "210171",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "254",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0639\u0641\u0648\u0646\u06cc \u062f\u0631 \u0628\u06cc\u0645\u0627\u0631\u0627\u0646 \u0645\u0628\u062a\u0644\u0627 \u0628\u0647 \u0646\u0642\u0635 \u0627\u06cc\u0645\u0646\u06cc \u0648 \u067e\u06cc\u0648\u0646\u062f"
            },
            {
              code: "210173",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "255",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0631\u0646\u06cc\u0647 \u0648 \u062e\u0627\u0631\u062c \u0686\u0634\u0645\u06cc"
            },
            {
              code: "210175",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "256",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0645\u0627\u062f\u0631\u0632\u0627\u062f\u06cc \u0642\u0644\u0628 \u062f\u0631 \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "210179",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "257",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u067e\u06cc\u0648\u0646\u062f \u0627\u0639\u0636\u0627\u0621 \u062f\u0627\u062e\u0644\u06cc \u0634\u06a9\u0645"
            },
            {
              code: "210180",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "258",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u062f\u0631 \u062c\u0631\u0627\u062d\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "210181",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "259",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u0642\u0644\u0628"
            },
            {
              code: "210182",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "260",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210186",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "261",
              name: "\u0628\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646"
            },
            {
              code: "210187",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "262",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "210188",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "263",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0686\u0634\u0645"
            },
            {
              code: "210190",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "264",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0645\u0648\u0644\u06a9\u0648\u0644\u0627\u0631\u0648\u0633\u06cc\u062a\u0648\u0698\u0646\u062a\u06cc\u06a9"
            },
            {
              code: "210203",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "265",
              name: "\u067e\u0631\u0648\u062a\u0632- \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "210205",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "266",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "210206",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "267",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u0645\u0627\u06a9\u0632\u06cc\u0644\u0648\u0641\u06cc\u0634\u06cc\u0627\u0644"
            },
            {
              code: "210208",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "268",
              name: "\u067e\u0631\u0648\u0633\u062a\u0648 \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "210209",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "269",
              name: "\u067e\u0631\u06cc\u0646\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210210",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "270",
              name: "\u067e\u0631\u06cc\u0648- \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "210218",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "271",
              name: "\u067e\u06cc\u0633 \u0645\u06cc\u06a9\u0631\u0648 \u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644- \u0627\u0644\u06a9\u062a\u0631\u0648\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210220",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "272",
              name: "\u067e\u06cc\u0634\u06af\u06cc\u0631\u06cc \u0648 \u06a9\u0646\u062a\u0631\u0644 \u0639\u0641\u0648\u0646\u062a\u200c\u0647\u0627\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc"
            },
            {
              code: "210221",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "273",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0628\u062f"
            },
            {
              code: "210222",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "274",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0644\u06cc\u0647"
            },
            {
              code: "210223",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "275",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0644\u06cc\u0647 \u0648 \u0627\u0648\u0631\u0648\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210226",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "276",
              name: "\u062a\u0631\u0648\u0645\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "210227",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "277",
              name: "\u062a\u0631\u0648\u0645\u0627\u06cc \u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "210238",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "278",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u0633\u062a\u0627\u0646"
            },
            {
              code: "210240",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "279",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u06cc\u0634\u0631\u0641\u062a\u0647- \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "210241",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "280",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "210242",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "281",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062a\u0631\u0648\u0645\u0627\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "210243",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "282",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0631\u0648\u0646 \u0628\u06cc\u0646 \u0632\u0646\u0627\u0646"
            },
            {
              code: "210244",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "283",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0633\u062a"
            },
            {
              code: "210246",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "284",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u062a\u0648\u0646 \u0641\u0642\u0631\u0627\u062a"
            },
            {
              code: "210247",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "285",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u0631 \u0648 \u06af\u0631\u062f\u0646"
            },
            {
              code: "210248",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "286",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u0631\u0637\u0627\u0646"
            },
            {
              code: "210249",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "209",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0631\u0648\u0642"
            },
            {
              code: "210251",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "287",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc \u062f\u0631\u0648\u0646 \u0628\u06cc\u0646- \u0644\u0627\u067e\u0627\u0631\u0627\u0633\u06a9\u0648\u067e\u06cc"
            },
            {
              code: "210252",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "288",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a \u06a9\u0631\u0627\u0646\u06cc\u0648\u0641\u0627\u0633\u06cc\u0627\u0644 \u0648 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210256",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "289",
              name: "\u062c\u0631\u0627\u062d\u06cc \u06a9\u0648\u0644\u0648\u0631\u06a9\u062a\u0627\u0644"
            },
            {
              code: "210258",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "290",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0632\u0627\u0646\u0648"
            },
            {
              code: "210259",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "291",
              name: "\u0686\u0634\u0645 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "210261",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "292",
              name: "\u0686\u0634\u0645 \u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646 \u0648 \u0627\u0646\u062d\u0631\u0627\u0641 \u0686\u0634\u0645"
            },
            {
              code: "210270",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "293",
              name: "\u062f\u0631\u062f"
            },
            {
              code: "210271",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "294",
              name: "\u062f\u0631\u062f\u0647\u0627\u06cc \u0645\u0632\u0645\u0646 \u062f\u0647\u0627\u0646\u06cc- \u0635\u0648\u0631\u062a\u06cc"
            },
            {
              code: "210272",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "295",
              name: "\u062f\u0631\u0645\u0627\u062a\u0648 \u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210273",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "296",
              name: "\u062f\u0631\u0645\u0627\u0646 \u0628\u06cc\u0646 \u0631\u0634\u062a\u0647\u200c\u200c\u0627\u06cc \u062a\u0631\u0645\u06cc\u0645\u200c\u0647\u0627\u06cc \u0632\u06cc\u0628\u0627\u06cc\u06cc \u0645\u0628\u062a\u0646\u06cc \u0628\u0631 \u0628\u0627\u0646\u062f\u06cc\u0646\u06af"
            },
            {
              code: "210276",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "297",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc- \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "210277",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "298",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc- \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210284",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "299",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u0645\u062f\u0627\u062e\u0644\u0647 \u0627\u06cc"
            },
            {
              code: "210291",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "300",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0646\u0638\u0627\u0645\u06cc"
            },
            {
              code: "210298",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "301",
              name: "\u0631\u06cc\u0646\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210309",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "302",
              name: "\u0633\u0631\u0637\u0627\u0646\u0647\u0627\u06cc \u062f\u0633\u062a\u06af\u0627\u0647 \u0627\u062f\u0631\u0627\u0631\u06cc- \u062a\u0646\u0627\u0633\u0644\u06cc"
            },
            {
              code: "210316",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "303",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc \u0648 \u0645\u0633\u0645\u0648\u0645\u06cc\u062a\u0647\u0627"
            },
            {
              code: "210318",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "304",
              name: "\u0633\u0648\u062e\u062a\u06af\u06cc"
            },
            {
              code: "210326",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "305",
              name: "\u0637\u0628 \u062a\u0633\u06a9\u06cc\u0646\u06cc"
            },
            {
              code: "210327",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "306",
              name: "\u0637\u0628 \u062e\u0648\u0627\u0628"
            },
            {
              code: "210328",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "307",
              name: "\u0637\u0628 \u0631\u0648\u0627\u0646 \u062a\u0646\u06cc"
            },
            {
              code: "210333",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "308",
              name: "\u0637\u0628 \u0645\u0627\u062f\u0631 \u0648 \u062c\u0646\u06cc\u0646"
            },
            {
              code: "210365",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "309",
              name: "\u0642\u0631\u0646\u06cc\u0647- \u0633\u06af\u0645\u0627\u0646 \u0642\u062f\u0627\u0645\u06cc"
            },
            {
              code: "210375",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "310",
              name: "\u06af\u0644\u0648\u06a9\u0648\u0645"
            },
            {
              code: "210379",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "311",
              name: "\u0644\u0627\u0631\u06cc\u0646\u06af\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "210380",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "312",
              name: "\u0644\u06cc\u0632\u0631"
            },
            {
              code: "210392",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "313",
              name: "\u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647 \u067e\u0632\u0634\u06a9\u06cc- ICU"
            },
            {
              code: "210393",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "314",
              name: "\u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210413",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "315",
              name: "\u0646\u0627\u0628\u0627\u0631\u0648\u0631\u06cc"
            },
            {
              code: "210414",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "316",
              name: "\u0646\u0627\u0631\u0633\u0627\u06cc\u06cc \u0642\u0644\u0628"
            },
            {
              code: "210415",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "317",
              name: "\u0646\u0627\u0632\u0627\u06cc\u06cc- IVF"
            },
            {
              code: "210420",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "318",
              name: "\u0646\u0648\u0631\u0648\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "210423",
              degree_id: "12",
              degree_name:
                "\u062f\u0627\u0646\u0634\u062c\u0648\u06cc \u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "319",
              name: "\u0648\u06cc\u062a\u0631\u0647 \u0648\u0631\u062a\u06cc\u0646- \u0633\u06af\u0645\u0627\u0646 \u062e\u0644\u0641\u06cc"
            },
            {
              code: "211101",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "230",
              name: "HIV/AIDS  \u0628\u0627\u0644\u06cc\u0646\u06cc"
            },
            {
              code: "211104",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "231",
              name: "\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0648 \u0646\u0648\u0631\u0648\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211105",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "232",
              name: "\u0627\u062e\u062a\u0644\u0627\u0644\u0627\u062a \u06a9\u0641 \u0644\u06af\u0646 \u062f\u0631 \u0632\u0646\u0627\u0646"
            },
            {
              code: "211107",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "233",
              name: "\u0627\u0631\u062a\u0648 \u0633\u0631\u062c\u0631\u06cc"
            },
            {
              code: "211109",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "234",
              name: "\u0627\u0631\u062a\u0648\u067e\u062f\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211113",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "235",
              name: "\u0627\u0631\u0648\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211114",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "236",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "211115",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "237",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc"
            },
            {
              code: "211116",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "238",
              name: "\u0627\u0631\u0648\u0644\u0648\u0698\u06cc \u0632\u0646\u0627\u0646 \u0648 \u0632\u0627\u06cc\u0645\u0627\u0646"
            },
            {
              code: "211117",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "239",
              name: "\u0627\u0633\u062a\u0631\u0627\u0628\u06cc\u0633\u0645"
            },
            {
              code: "211118",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "240",
              name: "\u0627\u0633\u062a\u0631\u06cc\u0648\u062a\u0627\u06a9\u0633\u06cc"
            },
            {
              code: "211124",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "241",
              name: "\u0627\u0642\u062f\u0627\u0645\u0627\u062a \u0645\u062f\u0627\u062e\u0644\u0647\u200c\u0627\u06cc \u0642\u0644\u0628 \u0628\u0632\u0631\u06af\u0633\u0627\u0644"
            },
            {
              code: "211125",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "242",
              name: "\u0627\u0642\u062f\u0627\u0645\u0627\u062a \u0645\u062f\u0627\u062e\u0644\u0647\u200c\u0627\u06cc \u0642\u0644\u0628 \u0648 \u0639\u0631\u0648\u0642 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211126",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "243",
              name: "\u0627\u06a9\u0648 \u06a9\u0627\u0631\u062f\u06cc\u0648\u06af\u0631\u0627\u0641\u06cc"
            },
            {
              code: "211127",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "244",
              name: "\u0627\u06a9\u0648\u0644\u0648 \u067e\u0644\u0627\u0633\u062a\u06cc\u06a9"
            },
            {
              code: "211131",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "245",
              name: "\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc \u0632\u0646\u0627\u0646"
            },
            {
              code: "211132",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "246",
              name: "\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "211134",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "247",
              name: "\u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "211139",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "248",
              name: "\u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644 \u06a9\u0627\u0631\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211140",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "249",
              name: "\u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644 \u06a9\u0627\u0631\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0631\u06a9\u0648\u062f\u06a9\u0627\u0646 \u0648\u0646\u0648\u062c\u0648\u0627\u0646\u0627\u0646"
            },
            {
              code: "211142",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "250",
              name: "\u0622\u0633\u06cc\u0628 \u0634\u0646\u0627\u0633\u06cc \u067e\u0648\u0633\u062a"
            },
            {
              code: "211148",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "251",
              name: "\u0622\u0646\u062f\u0648\u06cc\u0648\u0631\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211164",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "252",
              name: "\u0628\u0648\u06cc \u0628\u062f \u062f\u0647\u0627\u0646"
            },
            {
              code: "211170",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "253",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0633\u0637\u062d \u0686\u0634\u0645"
            },
            {
              code: "211171",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "254",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0639\u0641\u0648\u0646\u06cc \u062f\u0631 \u0628\u06cc\u0645\u0627\u0631\u0627\u0646 \u0645\u0628\u062a\u0644\u0627 \u0628\u0647 \u0646\u0642\u0635 \u0627\u06cc\u0645\u0646\u06cc \u0648 \u067e\u06cc\u0648\u0646\u062f"
            },
            {
              code: "211173",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "255",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0642\u0631\u0646\u06cc\u0647 \u0648 \u062e\u0627\u0631\u062c \u0686\u0634\u0645\u06cc"
            },
            {
              code: "211175",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "256",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc\u0647\u0627\u06cc \u0645\u0627\u062f\u0631\u0632\u0627\u062f\u06cc \u0642\u0644\u0628 \u062f\u0631 \u0628\u0627\u0644\u063a\u06cc\u0646"
            },
            {
              code: "211179",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "257",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u067e\u06cc\u0648\u0646\u062f \u0627\u0639\u0636\u0627\u0621 \u062f\u0627\u062e\u0644\u06cc \u0634\u06a9\u0645"
            },
            {
              code: "211180",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "258",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u062f\u0631 \u062c\u0631\u0627\u062d\u06cc \u0645\u063a\u0632 \u0648 \u0627\u0639\u0635\u0627\u0628"
            },
            {
              code: "211181",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "259",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u0642\u0644\u0628"
            },
            {
              code: "211182",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "260",
              name: "\u0628\u06cc\u0647\u0648\u0634\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211186",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "261",
              name: "\u0628\u06cc\u0648\u0644\u0648\u0698\u06cc \u062f\u0647\u0627\u0646"
            },
            {
              code: "211187",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "262",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "211188",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "263",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0686\u0634\u0645"
            },
            {
              code: "211190",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "264",
              name: "\u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc \u0645\u0648\u0644\u06a9\u0648\u0644\u0627\u0631\u0648\u0633\u06cc\u062a\u0648\u0698\u0646\u062a\u06cc\u06a9"
            },
            {
              code: "211203",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "320",
              name: "\u067e\u0631\u0648\u062a\u0632-\u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "211205",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "266",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "211206",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "267",
              name: "\u067e\u0631\u0648\u062a\u0632\u0647\u0627\u06cc \u0645\u0627\u06a9\u0632\u06cc\u0644\u0648\u0641\u06cc\u0634\u06cc\u0627\u0644"
            },
            {
              code: "211208",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "268",
              name: "\u067e\u0631\u0648\u0633\u062a\u0648 \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "211209",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "269",
              name: "\u067e\u0631\u06cc\u0646\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211210",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "270",
              name: "\u067e\u0631\u06cc\u0648- \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "211218",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "271",
              name: "\u067e\u06cc\u0633 \u0645\u06cc\u06a9\u0631\u0648 \u0627\u06cc\u0646\u062a\u0631\u0648\u0646\u0634\u0646\u0627\u0644- \u0627\u0644\u06a9\u062a\u0631\u0648\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211220",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "272",
              name: "\u067e\u06cc\u0634\u06af\u06cc\u0631\u06cc \u0648 \u06a9\u0646\u062a\u0631\u0644 \u0639\u0641\u0648\u0646\u062a\u200c\u0647\u0627\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc"
            },
            {
              code: "211221",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "273",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0628\u062f"
            },
            {
              code: "211222",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "274",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0644\u06cc\u0647"
            },
            {
              code: "211223",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "275",
              name: "\u067e\u06cc\u0648\u0646\u062f \u06a9\u0644\u06cc\u0647 \u0648 \u0627\u0648\u0631\u0648\u0627\u0646\u06a9\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211226",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "276",
              name: "\u062a\u0631\u0648\u0645\u0627\u06cc \u062f\u0646\u062f\u0627\u0646\u06cc"
            },
            {
              code: "211227",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "277",
              name: "\u062a\u0631\u0648\u0645\u0627\u06cc \u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "211238",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "278",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u0633\u062a\u0627\u0646"
            },
            {
              code: "211240",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "279",
              name: "\u062c\u0631\u0627\u062d\u06cc \u067e\u06cc\u0634\u0631\u0641\u062a\u0647- \u0627\u06cc\u0645\u067e\u0644\u0646\u062a"
            },
            {
              code: "211241",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "280",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062a\u0631\u0645\u06cc\u0645\u06cc \u062f\u0647\u0627\u0646 \u0648 \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "211242",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "281",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062a\u0631\u0648\u0645\u0627\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a"
            },
            {
              code: "211243",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "282",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0631\u0648\u0646 \u0628\u06cc\u0646 \u0632\u0646\u0627\u0646"
            },
            {
              code: "211244",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "283",
              name: "\u062c\u0631\u0627\u062d\u06cc \u062f\u0633\u062a"
            },
            {
              code: "211246",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "284",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u062a\u0648\u0646 \u0641\u0642\u0631\u0627\u062a"
            },
            {
              code: "211247",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "285",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u0631 \u0648 \u06af\u0631\u062f\u0646"
            },
            {
              code: "211248",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "286",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0633\u0631\u0637\u0627\u0646"
            },
            {
              code: "211249",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "209",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0631\u0648\u0642"
            },
            {
              code: "211251",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "287",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0639\u0645\u0648\u0645\u06cc \u062f\u0631\u0648\u0646 \u0628\u06cc\u0646- \u0644\u0627\u067e\u0627\u0631\u0627\u0633\u06a9\u0648\u067e\u06cc"
            },
            {
              code: "211252",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "288",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0641\u06a9 \u0648 \u0635\u0648\u0631\u062a \u06a9\u0631\u0627\u0646\u06cc\u0648\u0641\u0627\u0633\u06cc\u0627\u0644 \u0648 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211256",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "289",
              name: "\u062c\u0631\u0627\u062d\u06cc \u06a9\u0648\u0644\u0648\u0631\u06a9\u062a\u0627\u0644"
            },
            {
              code: "211258",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "290",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0632\u0627\u0646\u0648"
            },
            {
              code: "211259",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "291",
              name: "\u0686\u0634\u0645 \u0627\u0637\u0641\u0627\u0644"
            },
            {
              code: "211261",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "292",
              name: "\u0686\u0634\u0645 \u067e\u0632\u0634\u06a9\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646 \u0648 \u0627\u0646\u062d\u0631\u0627\u0641 \u0686\u0634\u0645"
            },
            {
              code: "211270",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "293",
              name: "\u062f\u0631\u062f"
            },
            {
              code: "211271",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "294",
              name: "\u062f\u0631\u062f\u0647\u0627\u06cc \u0645\u0632\u0645\u0646 \u062f\u0647\u0627\u0646\u06cc- \u0635\u0648\u0631\u062a\u06cc"
            },
            {
              code: "211272",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "295",
              name: "\u062f\u0631\u0645\u0627\u062a\u0648 \u067e\u0627\u062a\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211273",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "296",
              name: "\u062f\u0631\u0645\u0627\u0646 \u0628\u06cc\u0646 \u0631\u0634\u062a\u0647\u200c\u200c\u0627\u06cc \u062a\u0631\u0645\u06cc\u0645\u200c\u0647\u0627\u06cc \u0632\u06cc\u0628\u0627\u06cc\u06cc \u0645\u0628\u062a\u0646\u06cc \u0628\u0631 \u0628\u0627\u0646\u062f\u06cc\u0646\u06af"
            },
            {
              code: "211276",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "297",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc- \u0639\u0645\u0648\u0645\u06cc"
            },
            {
              code: "211277",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "298",
              name: "\u062f\u0646\u062f\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0628\u06cc\u0645\u0627\u0631\u0633\u062a\u0627\u0646\u06cc- \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211284",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "299",
              name: "\u0631\u0627\u062f\u06cc\u0648\u0644\u0648\u0698\u06cc \u0645\u062f\u0627\u062e\u0644\u0647 \u0627\u06cc"
            },
            {
              code: "211291",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "300",
              name: "\u0631\u0648\u0627\u0646\u067e\u0632\u0634\u06a9\u06cc \u0646\u0638\u0627\u0645\u06cc"
            },
            {
              code: "211298",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "301",
              name: "\u0631\u06cc\u0646\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211309",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "302",
              name: "\u0633\u0631\u0637\u0627\u0646\u0647\u0627\u06cc \u062f\u0633\u062a\u06af\u0627\u0647 \u0627\u062f\u0631\u0627\u0631\u06cc- \u062a\u0646\u0627\u0633\u0644\u06cc"
            },
            {
              code: "211316",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "303",
              name: "\u0633\u0645 \u0634\u0646\u0627\u0633\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc \u0648 \u0645\u0633\u0645\u0648\u0645\u06cc\u062a\u0647\u0627"
            },
            {
              code: "211318",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "304",
              name: "\u0633\u0648\u062e\u062a\u06af\u06cc"
            },
            {
              code: "211326",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "305",
              name: "\u0637\u0628 \u062a\u0633\u06a9\u06cc\u0646\u06cc"
            },
            {
              code: "211327",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "306",
              name: "\u0637\u0628 \u062e\u0648\u0627\u0628"
            },
            {
              code: "211328",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "307",
              name: "\u0637\u0628 \u0631\u0648\u0627\u0646 \u062a\u0646\u06cc"
            },
            {
              code: "211333",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "308",
              name: "\u0637\u0628 \u0645\u0627\u062f\u0631 \u0648 \u062c\u0646\u06cc\u0646"
            },
            {
              code: "211365",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "309",
              name: "\u0642\u0631\u0646\u06cc\u0647- \u0633\u06af\u0645\u0627\u0646 \u0642\u062f\u0627\u0645\u06cc"
            },
            {
              code: "211375",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "310",
              name: "\u06af\u0644\u0648\u06a9\u0648\u0645"
            },
            {
              code: "211379",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "311",
              name: "\u0644\u0627\u0631\u06cc\u0646\u06af\u0648\u0644\u0648\u0698\u06cc"
            },
            {
              code: "211380",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "312",
              name: "\u0644\u06cc\u0632\u0631"
            },
            {
              code: "211392",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "313",
              name: "\u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647 \u067e\u0632\u0634\u06a9\u06cc- ICU"
            },
            {
              code: "211393",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "314",
              name: "\u0645\u0631\u0627\u0642\u0628\u062a \u0647\u0627\u06cc \u0648\u06cc\u0698\u0647 \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211413",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "315",
              name: "\u0646\u0627\u0628\u0627\u0631\u0648\u0631\u06cc"
            },
            {
              code: "211414",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "316",
              name: "\u0646\u0627\u0631\u0633\u0627\u06cc\u06cc \u0642\u0644\u0628"
            },
            {
              code: "211415",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "317",
              name: "\u0646\u0627\u0632\u0627\u06cc\u06cc- IVF"
            },
            {
              code: "211420",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "318",
              name: "\u0646\u0648\u0631\u0648\u0641\u06cc\u0632\u06cc\u0648\u0644\u0648\u0698\u06cc \u0628\u0627\u0644\u06cc\u0646\u06cc \u06a9\u0648\u062f\u06a9\u0627\u0646"
            },
            {
              code: "211423",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "319",
              name: "\u0648\u06cc\u062a\u0631\u0647 \u0648\u0631\u062a\u06cc\u0646- \u0633\u06af\u0645\u0627\u0646 \u062e\u0644\u0641\u06cc"
            },
            {
              code: "211425",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "321",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc \u0635\u0631\u0639"
            },
            {
              code: "211426",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "322",
              name: "\u0628\u06cc\u0645\u0627\u0631\u06cc MS"
            },
            {
              code: "211427",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "323",
              name: "\u06a9\u0627\u0634\u062a \u062d\u0644\u0632\u0648\u0646"
            },
            {
              code: "211428",
              degree_id: "13",
              degree_name: "\u0641\u0644\u0648\u0634\u06cc\u067e",
              expertise_id: "324",
              name: "\u062c\u0631\u0627\u062d\u06cc \u0634\u0627\u0646\u0647"
            },
            {
              code: "0",
              degree_id: "14",
              degree_name: "\u0646\u0627\u0645\u0634\u062e\u0635",
              expertise_id: "325",
              name: "\u0646\u0627\u0645\u0634\u062e\u0635"
            },
            {
              code: "0",
              degree_id: "15",
              degree_name: "\u0645\u0634\u0627\u0648\u0631",
              expertise_id: "326",
              name: "\u0633\u0644\u0627\u0645\u062a \u062c\u0646\u0633\u06cc"
            }
          ]
        },
        Object.fromEntries(
          Object.entries(props.args).filter(([_, v]) => v !== undefined)
        )
      ),
    [props.args]
  );

  const $props = {
    ...args,
    ...variants
  };

  const __nextRouter = useNextRouter();

  const $ctx = useDataEnv?.() || {};
  const refsRef = React.useRef({});
  const $refs = refsRef.current;

  const $globalActions = useGlobalActions?.();

  const stateSpecs: Parameters<typeof useDollarState>[0] = React.useMemo(
    () => [
      {
        path: "expertiseArray",
        type: "private",
        variableType: "array",
        initFunc: ({ $props, $state, $queries, $ctx }) => []
      },
      {
        path: "profileExpertiseItem[].degreeValue",
        type: "private",
        variableType: "text"
      },
      {
        path: "profileExpertiseItem2[].degreeValue",
        type: "private",
        variableType: "text"
      },
      {
        path: "profileExpertiseItem[].expertiseValue",
        type: "private",
        variableType: "text"
      },
      {
        path: "profileExpertiseItem2[].expertiseValue",
        type: "private",
        variableType: "text"
      },
      {
        path: "profileExpertiseItem[].expertiseTitle",
        type: "private",
        variableType: "text"
      },
      {
        path: "profileExpertiseItem2[].expertiseTitle",
        type: "private",
        variableType: "text"
      },
      {
        path: "dialog.open",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined
      },
      {
        path: "selectedId",
        type: "private",
        variableType: "text",
        initFunc: ({ $props, $state, $queries, $ctx }) => ""
      },
      {
        path: "changedExperies",
        type: "private",
        variableType: "array",
        initFunc: ({ $props, $state, $queries, $ctx }) => []
      },
      {
        path: "authApi.data",
        type: "private",
        variableType: "object",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "authApi"
      },
      {
        path: "authApi.error",
        type: "private",
        variableType: "object",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "authApi"
      },
      {
        path: "authApi.loading",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "authApi"
      },
      {
        path: "expertisesApi.data",
        type: "private",
        variableType: "object",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "expertisesApi"
      },
      {
        path: "expertisesApi.error",
        type: "private",
        variableType: "object",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "expertisesApi"
      },
      {
        path: "expertisesApi.loading",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => undefined,

        refName: "expertisesApi"
      },
      {
        path: "isLoading",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => false
      },
      {
        path: "isLoadingDelete",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => false
      },
      {
        path: "isLoadingSave",
        type: "private",
        variableType: "boolean",
        initFunc: ({ $props, $state, $queries, $ctx }) => false
      }
    ],
    [$props, $ctx, $refs]
  );
  const $state = useDollarState(stateSpecs, {
    $props,
    $ctx,
    $queries: {},
    $refs
  });

  const styleTokensClassNames = _useStyleTokens();

  return (
    <div
      data-plasmic-name={"root"}
      data-plasmic-override={overrides.root}
      data-plasmic-root={true}
      data-plasmic-for-node={forNode}
      className={classNames(
        projectcss.all,
        projectcss.root_reset,
        projectcss.plasmic_default_styles,
        projectcss.plasmic_mixins,
        styleTokensClassNames,
        sty.root
      )}
    >
      <ApiRequest
        data-plasmic-name={"authApi"}
        data-plasmic-override={overrides.authApi}
        className={classNames("__wab_instance", sty.authApi)}
        errorDisplay={null}
        loadingDisplay={
          <div className={classNames(projectcss.all, sty.freeBox__bhGCx)}>
            <Icon34Icon
              className={classNames(projectcss.all, sty.svg__lYhYn)}
              role={"img"}
            />
          </div>
        }
        method={"GET"}
        onError={async (...eventArgs: any) => {
          generateStateOnChangeProp($state, ["authApi", "error"]).apply(
            null,
            eventArgs
          );
        }}
        onLoading={async (...eventArgs: any) => {
          generateStateOnChangeProp($state, ["authApi", "loading"]).apply(
            null,
            eventArgs
          );
        }}
        onSuccess={async (...eventArgs: any) => {
          generateStateOnChangeProp($state, ["authApi", "data"]).apply(
            null,
            eventArgs
          );
        }}
        ref={ref => {
          $refs["authApi"] = ref;
        }}
        url={"https://api.paziresh24.com/V1/auth/me"}
      >
        <ApiRequest
          data-plasmic-name={"expertisesApi"}
          data-plasmic-override={overrides.expertisesApi}
          className={classNames("__wab_instance", sty.expertisesApi)}
          errorDisplay={null}
          loadingDisplay={
            <Icon34Icon
              className={classNames(projectcss.all, sty.svg__wNjv6)}
              role={"img"}
            />
          }
          method={"GET"}
          onError={async (...eventArgs: any) => {
            generateStateOnChangeProp($state, ["expertisesApi", "error"]).apply(
              null,
              eventArgs
            );
          }}
          onLoading={async (...eventArgs: any) => {
            generateStateOnChangeProp($state, [
              "expertisesApi",
              "loading"
            ]).apply(null, eventArgs);
          }}
          onSuccess={async (...eventArgs: any) => {
            generateStateOnChangeProp($state, ["expertisesApi", "data"]).apply(
              null,
              eventArgs
            );

            (async data => {
              const $steps = {};

              $steps["sendLog"] =
                !$state.expertisesApi.data.data ||
                !$state.expertisesApi.data.data.length
                  ? (() => {
                      const actionArgs = {
                        args: [
                          (() => {
                            try {
                              return {
                                event_group: "edit-profile",
                                data: {
                                  username: $state.authApi.data.data.username,
                                  userid: $state.authApi.data.data.id,
                                  userinfoid: $state.authApi.data.data.user_id
                                },
                                event_type: "no-expertise"
                              };
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })()
                        ]
                      };
                      return $globalActions["Splunk.sendLog"]?.apply(null, [
                        ...actionArgs.args
                      ]);
                    })()
                  : undefined;
              if (
                $steps["sendLog"] != null &&
                typeof $steps["sendLog"] === "object" &&
                typeof $steps["sendLog"].then === "function"
              ) {
                $steps["sendLog"] = await $steps["sendLog"];
              }
            }).apply(null, eventArgs);
          }}
          ref={ref => {
            $refs["expertisesApi"] = ref;
          }}
          url={(() => {
            try {
              return `https://api.paziresh24.com/V1/doctor/expertises`;
            } catch (e) {
              if (
                e instanceof TypeError ||
                e?.plasmicType === "PlasmicUndefinedDataError"
              ) {
                return undefined;
              }
              throw e;
            }
          })()}
        >
          {(_par => (!_par ? [] : Array.isArray(_par) ? _par : [_par]))(
            (() => {
              try {
                return $state.expertisesApi.data.data;
              } catch (e) {
                if (
                  e instanceof TypeError ||
                  e?.plasmicType === "PlasmicUndefinedDataError"
                ) {
                  return [];
                }
                throw e;
              }
            })()
          ).map((__plasmic_item_0, __plasmic_idx_0) => {
            const currentItem = __plasmic_item_0;
            const currentIndex = __plasmic_idx_0;
            return (() => {
              const child$Props = {
                className: classNames(
                  "__wab_instance",
                  sty.profileExpertiseItem
                ),
                degree: (() => {
                  try {
                    return $props.degree;
                  } catch (e) {
                    if (
                      e instanceof TypeError ||
                      e?.plasmicType === "PlasmicUndefinedDataError"
                    ) {
                      return undefined;
                    }
                    throw e;
                  }
                })(),
                degreeValue: generateStateValueProp($state, [
                  "profileExpertiseItem",
                  __plasmic_idx_0,
                  "degreeValue"
                ]),
                expertise: (() => {
                  try {
                    return $props.expertise;
                  } catch (e) {
                    if (
                      e instanceof TypeError ||
                      e?.plasmicType === "PlasmicUndefinedDataError"
                    ) {
                      return undefined;
                    }
                    throw e;
                  }
                })(),
                expertiseTitle: generateStateValueProp($state, [
                  "profileExpertiseItem",
                  __plasmic_idx_0,
                  "expertiseTitle"
                ]),
                expertiseValue: generateStateValueProp($state, [
                  "profileExpertiseItem",
                  __plasmic_idx_0,
                  "expertiseValue"
                ]),
                id: (() => {
                  try {
                    return currentItem.id;
                  } catch (e) {
                    if (
                      e instanceof TypeError ||
                      e?.plasmicType === "PlasmicUndefinedDataError"
                    ) {
                      return undefined;
                    }
                    throw e;
                  }
                })(),
                key: currentIndex,
                onDegreeValueChange: async (...eventArgs: any) => {
                  generateStateOnChangeProp($state, [
                    "profileExpertiseItem",
                    __plasmic_idx_0,
                    "degreeValue"
                  ]).apply(null, eventArgs);

                  if (
                    eventArgs.length > 1 &&
                    eventArgs[1] &&
                    eventArgs[1]._plasmic_state_init_
                  ) {
                    return;
                  }

                  (async val => {
                    const $steps = {};

                    $steps["updateChangedExperies"] = true
                      ? (() => {
                          const actionArgs = {
                            variable: {
                              objRoot: $state,
                              variablePath: ["changedExperies"]
                            },
                            operation: 0,
                            value: (() => {
                              const itemExists = $state.changedExperies.some(
                                item => item.id === currentItem.id
                              );
                              if (itemExists) {
                                return $state.changedExperies.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        ...item,
                                        degree_id: val
                                      }
                                    : item
                                );
                              } else {
                                return [
                                  ...$state.changedExperies,
                                  {
                                    id: currentItem.id,
                                    degree_id: val,
                                    alias_title: currentItem.alias_title,
                                    expertise_id: currentItem.expertise.id
                                  }
                                ];
                              }
                            })()
                          };
                          return (({
                            variable,
                            value,
                            startIndex,
                            deleteCount
                          }) => {
                            if (!variable) {
                              return;
                            }
                            const { objRoot, variablePath } = variable;

                            $stateSet(objRoot, variablePath, value);
                            return value;
                          })?.apply(null, [actionArgs]);
                        })()
                      : undefined;
                    if (
                      $steps["updateChangedExperies"] != null &&
                      typeof $steps["updateChangedExperies"] === "object" &&
                      typeof $steps["updateChangedExperies"].then === "function"
                    ) {
                      $steps["updateChangedExperies"] =
                        await $steps["updateChangedExperies"];
                    }
                  }).apply(null, eventArgs);
                },
                onDeleteClick: async id => {
                  const $steps = {};

                  $steps["updateDialogOpen"] = true
                    ? (() => {
                        const actionArgs = {
                          variable: {
                            objRoot: $state,
                            variablePath: ["dialog", "open"]
                          },
                          operation: 4
                        };
                        return (({
                          variable,
                          value,
                          startIndex,
                          deleteCount
                        }) => {
                          if (!variable) {
                            return;
                          }
                          const { objRoot, variablePath } = variable;

                          const oldValue = $stateGet(objRoot, variablePath);
                          $stateSet(objRoot, variablePath, !oldValue);
                          return !oldValue;
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["updateDialogOpen"] != null &&
                    typeof $steps["updateDialogOpen"] === "object" &&
                    typeof $steps["updateDialogOpen"].then === "function"
                  ) {
                    $steps["updateDialogOpen"] =
                      await $steps["updateDialogOpen"];
                  }

                  $steps["updateSelectedId"] = true
                    ? (() => {
                        const actionArgs = {
                          variable: {
                            objRoot: $state,
                            variablePath: ["selectedId"]
                          },
                          operation: 0,
                          value: id
                        };
                        return (({
                          variable,
                          value,
                          startIndex,
                          deleteCount
                        }) => {
                          if (!variable) {
                            return;
                          }
                          const { objRoot, variablePath } = variable;

                          $stateSet(objRoot, variablePath, value);
                          return value;
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["updateSelectedId"] != null &&
                    typeof $steps["updateSelectedId"] === "object" &&
                    typeof $steps["updateSelectedId"].then === "function"
                  ) {
                    $steps["updateSelectedId"] =
                      await $steps["updateSelectedId"];
                  }
                },
                onExpertiseTitleChange: async (...eventArgs: any) => {
                  generateStateOnChangeProp($state, [
                    "profileExpertiseItem",
                    __plasmic_idx_0,
                    "expertiseTitle"
                  ]).apply(null, eventArgs);

                  if (
                    eventArgs.length > 1 &&
                    eventArgs[1] &&
                    eventArgs[1]._plasmic_state_init_
                  ) {
                    return;
                  }

                  (async val => {
                    const $steps = {};

                    $steps["updateChangedExperies"] = true
                      ? (() => {
                          const actionArgs = {
                            variable: {
                              objRoot: $state,
                              variablePath: ["changedExperies"]
                            },
                            operation: 0,
                            value: (() => {
                              const itemExists = $state.changedExperies.some(
                                item => item.id === currentItem.id
                              );
                              if (itemExists) {
                                return $state.changedExperies.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        ...item,
                                        alias_title: val
                                      }
                                    : item
                                );
                              } else {
                                return [
                                  ...$state.changedExperies,
                                  {
                                    id: currentItem.id,
                                    degree_id: currentItem.degree.id,
                                    alias_title: val,
                                    expertise_id: currentItem.expertise.id
                                  }
                                ];
                              }
                            })()
                          };
                          return (({
                            variable,
                            value,
                            startIndex,
                            deleteCount
                          }) => {
                            if (!variable) {
                              return;
                            }
                            const { objRoot, variablePath } = variable;

                            $stateSet(objRoot, variablePath, value);
                            return value;
                          })?.apply(null, [actionArgs]);
                        })()
                      : undefined;
                    if (
                      $steps["updateChangedExperies"] != null &&
                      typeof $steps["updateChangedExperies"] === "object" &&
                      typeof $steps["updateChangedExperies"].then === "function"
                    ) {
                      $steps["updateChangedExperies"] =
                        await $steps["updateChangedExperies"];
                    }
                  }).apply(null, eventArgs);
                },
                onExpertiseValueChange: async (...eventArgs: any) => {
                  generateStateOnChangeProp($state, [
                    "profileExpertiseItem",
                    __plasmic_idx_0,
                    "expertiseValue"
                  ]).apply(null, eventArgs);

                  if (
                    eventArgs.length > 1 &&
                    eventArgs[1] &&
                    eventArgs[1]._plasmic_state_init_
                  ) {
                    return;
                  }

                  (async val => {
                    const $steps = {};

                    $steps["updateChangedExperies"] = true
                      ? (() => {
                          const actionArgs = {
                            variable: {
                              objRoot: $state,
                              variablePath: ["changedExperies"]
                            },
                            operation: 0,
                            value: (() => {
                              const itemExists = $state.changedExperies.some(
                                item => item.id === currentItem.id
                              );
                              if (itemExists) {
                                return $state.changedExperies.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        ...item,
                                        expertise_id: val
                                      }
                                    : item
                                );
                              } else {
                                return [
                                  ...$state.changedExperies,
                                  {
                                    id: currentItem.id,
                                    degree_id: currentItem.degree.id,
                                    alias_title: currentItem.alias_title,
                                    expertise_id: val
                                  }
                                ];
                              }
                            })()
                          };
                          return (({
                            variable,
                            value,
                            startIndex,
                            deleteCount
                          }) => {
                            if (!variable) {
                              return;
                            }
                            const { objRoot, variablePath } = variable;

                            $stateSet(objRoot, variablePath, value);
                            return value;
                          })?.apply(null, [actionArgs]);
                        })()
                      : undefined;
                    if (
                      $steps["updateChangedExperies"] != null &&
                      typeof $steps["updateChangedExperies"] === "object" &&
                      typeof $steps["updateChangedExperies"].then === "function"
                    ) {
                      $steps["updateChangedExperies"] =
                        await $steps["updateChangedExperies"];
                    }
                  }).apply(null, eventArgs);
                }
              };

              initializePlasmicStates(
                $state,
                [
                  {
                    name: "profileExpertiseItem[].degreeValue",
                    initFunc: ({ $props, $state, $queries }) =>
                      (() => {
                        try {
                          return currentItem.degree.id;
                        } catch (e) {
                          if (
                            e instanceof TypeError ||
                            e?.plasmicType === "PlasmicUndefinedDataError"
                          ) {
                            return undefined;
                          }
                          throw e;
                        }
                      })()
                  },
                  {
                    name: "profileExpertiseItem[].expertiseValue",
                    initFunc: ({ $props, $state, $queries }) =>
                      (() => {
                        try {
                          return currentItem.expertise.id;
                        } catch (e) {
                          if (
                            e instanceof TypeError ||
                            e?.plasmicType === "PlasmicUndefinedDataError"
                          ) {
                            return undefined;
                          }
                          throw e;
                        }
                      })()
                  },
                  {
                    name: "profileExpertiseItem[].expertiseTitle",
                    initFunc: ({ $props, $state, $queries }) =>
                      (() => {
                        try {
                          return currentItem.alias_title;
                        } catch (e) {
                          if (
                            e instanceof TypeError ||
                            e?.plasmicType === "PlasmicUndefinedDataError"
                          ) {
                            return undefined;
                          }
                          throw e;
                        }
                      })()
                  }
                ],
                [__plasmic_idx_0]
              );
              return (
                <ProfileExpertiseItem
                  data-plasmic-name={"profileExpertiseItem"}
                  data-plasmic-override={overrides.profileExpertiseItem}
                  {...child$Props}
                />
              );
            })();
          })}
          <div className={classNames(projectcss.all, sty.freeBox__deNqh)}>
            {(() => {
              try {
                return (
                  !$state.expertisesApi.data.data ||
                  !$state.expertisesApi.data.data.length
                );
              } catch (e) {
                if (
                  e instanceof TypeError ||
                  e?.plasmicType === "PlasmicUndefinedDataError"
                ) {
                  return false;
                }
                throw e;
              }
            })() ? (
              <div className={classNames(projectcss.all, sty.freeBox___3PSHh)}>
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__f7WFc
                  )}
                >
                  <React.Fragment>
                    <React.Fragment>{""}</React.Fragment>
                    {
                      <h6
                        data-plasmic-name={"h6"}
                        data-plasmic-override={overrides.h6}
                        className={classNames(
                          projectcss.all,
                          projectcss.h6,
                          projectcss.__wab_text,
                          sty.h6
                        )}
                      >
                        <React.Fragment>
                          <React.Fragment>
                            {"\u26a0\ufe0f \u062a\u0648\u062c\u0647: "}
                          </React.Fragment>
                          <span
                            className={
                              "plasmic_default__all plasmic_default__span"
                            }
                            style={{ fontWeight: 700 }}
                          >
                            {"\u0647\u06cc\u0686"}
                          </span>
                          <React.Fragment> </React.Fragment>
                          <span
                            className={
                              "plasmic_default__all plasmic_default__span"
                            }
                            style={{ fontWeight: 700 }}
                          >
                            {"\u062a\u062e\u0635\u0635\u06cc"}
                          </span>
                          <React.Fragment>
                            {
                              " \u0628\u0631\u0627\u06cc \u0634\u0645\u0627 \u062b\u0628\u062a "
                            }
                          </React.Fragment>
                          <span
                            className={
                              "plasmic_default__all plasmic_default__span"
                            }
                            style={{ fontWeight: 700 }}
                          >
                            {"\u0646\u0634\u062f\u0647"}
                          </span>
                          <React.Fragment>
                            {" \u0627\u0633\u062a."}
                          </React.Fragment>
                        </React.Fragment>
                      </h6>
                    }
                    <React.Fragment>
                      {
                        "\u0628\u0631\u0627\u06cc \u0646\u0645\u0627\u06cc\u0634 \u067e\u0631\u0648\u0641\u0627\u06cc\u0644 \u0634\u0645\u0627 \u0628\u0647 \u0628\u06cc\u0645\u0627\u0631\u0627\u0646\u060c \u062b\u0628\u062a \u062d\u062f\u0627\u0642\u0644 \u06cc\u06a9 \u062a\u062e\u0635\u0635 "
                      }
                    </React.Fragment>
                    <span
                      className={"plasmic_default__all plasmic_default__span"}
                      style={{ fontWeight: 700 }}
                    >
                      {"\u0636\u0631\u0648\u0631\u06cc"}
                    </span>
                    <React.Fragment>
                      {
                        " \u0627\u0633\u062a.\n\ud83d\udccb \u0644\u0637\u0641\u0627\u064b \u0628\u0627 \u0627\u0646\u062a\u062e\u0627\u0628 \u06af\u0632\u06cc\u0646\u0647 + \u062a\u062e\u0635\u0635 \u062e\u0648\u062f \u0631\u0627 \u0627\u0636\u0627\u0641\u0647 \u0648 \u062b\u0628\u062a \u06a9\u0646\u06cc\u062f."
                      }
                    </React.Fragment>
                  </React.Fragment>
                </div>
              </div>
            ) : null}
            <div className={classNames(projectcss.all, sty.freeBox__afDni)}>
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__gaqai
                )}
              >
                <React.Fragment>
                  <span
                    className={"plasmic_default__all plasmic_default__span"}
                    style={{ color: "#000000" }}
                  >
                    {
                      "\u0639\u0646\u0648\u0627\u0646 \u062a\u062e\u0635\u0635 \u0628\u0627\u06cc\u062f \u0628\u0627 \u0646\u0627\u0645 \u062f\u0642\u06cc\u0642 \u0648 \u0631\u0633\u0645\u06cc \u062a\u062e\u0635\u0635 \u0622\u063a\u0627\u0632 \u0634\u0648\u062f (\u0645\u0627\u0646\u0646\u062f: \u00ab\u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0645\u0627\u0645\u0627\u06cc\u06cc\u00bb)."
                    }
                  </span>
                  <React.Fragment>
                    {
                      "\n\u0628\u0644\u0627\u0641\u0627\u0635\u0644\u0647 \u067e\u0633 \u0627\u0632 \u0622\u0646\u060c \u062a\u0648\u0636\u06cc\u062d\u06cc \u06a9\u0648\u062a\u0627\u0647 \u0648 \u0631\u0648\u0634\u0646 \u062f\u0631\u0628\u0627\u0631\u0647 \u062e\u062f\u0645\u0627\u062a \u0627\u0631\u0627\u0626\u0647 \u06af\u0631\u062f\u062f.\n\u0627\u0633\u062a\u0641\u0627\u062f\u0647 \u0627\u0632 \u062c\u062f\u0627\u06a9\u0646\u0646\u062f\u0647 \u0645\u0646\u0627\u0633\u0628 \u0645\u0627\u0646\u0646\u062f \u00ab | \u00bb \u0648 \u0631\u0639\u0627\u06cc\u062a \u0641\u0627\u0635\u0644\u0647\u200c\u06af\u0630\u0627\u0631\u06cc \u0635\u062d\u06cc\u062d\u060c \u062e\u0648\u0627\u0646\u0627\u06cc\u06cc \u0645\u062a\u0646 \u0631\u0627 \u0627\u0641\u0632\u0627\u06cc\u0634 \u0645\u06cc\u200c\u062f\u0647\u062f.\n\u0627\u0632 \u0628\u0647\u200c\u06a9\u0627\u0631 \u0628\u0631\u062f\u0646 \u0648\u0627\u0698\u06af\u0627\u0646\u06cc \u06a9\u0647 \u0645\u0648\u062c\u0628 \u062a\u0634\u0627\u0628\u0647 \u0628\u0627 \u0633\u0627\u06cc\u0631 \u062a\u062e\u0635\u0635\u200c\u0647\u0627 \u0645\u06cc\u200c\u0634\u0648\u062f\u060c \u067e\u0631\u0647\u06cc\u0632 \u0634\u0648\u062f."
                    }
                  </React.Fragment>
                </React.Fragment>
              </div>
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text___97Bgt
                )}
              >
                {
                  "\u0645\u062b\u0627\u0644  : \u06a9\u0627\u0631\u0634\u0646\u0627\u0633 \u0645\u0627\u0645\u0627\u06cc\u06cc | \u0632\u06af\u06cc\u0644 \u062a\u0646\u0627\u0633\u0644\u06cc | \u062a\u0628\u062e\u0627\u0644 \u062a\u0646\u0627\u0633\u0644\u06cc | \u0628\u06a9\u0627\u0631\u062a | \u0639\u0641\u0648\u0646\u062a \u0648\u0627\u0698\u0646"
                }
              </div>
            </div>
            <div className={classNames(projectcss.all, sty.freeBox__hKrof)}>
              {(_par => (!_par ? [] : Array.isArray(_par) ? _par : [_par]))(
                (() => {
                  try {
                    return $state.expertiseArray;
                  } catch (e) {
                    if (
                      e instanceof TypeError ||
                      e?.plasmicType === "PlasmicUndefinedDataError"
                    ) {
                      return [];
                    }
                    throw e;
                  }
                })()
              ).map((__plasmic_item_0, __plasmic_idx_0) => {
                const currentItem = __plasmic_item_0;
                const currentIndex = __plasmic_idx_0;
                return (() => {
                  const child$Props = {
                    className: classNames(
                      "__wab_instance",
                      sty.profileExpertiseItem2
                    ),
                    degree: (() => {
                      try {
                        return $props.degree;
                      } catch (e) {
                        if (
                          e instanceof TypeError ||
                          e?.plasmicType === "PlasmicUndefinedDataError"
                        ) {
                          return undefined;
                        }
                        throw e;
                      }
                    })(),
                    degreeValue: generateStateValueProp($state, [
                      "profileExpertiseItem2",
                      __plasmic_idx_0,
                      "degreeValue"
                    ]),
                    expertise: (() => {
                      try {
                        return $props.expertise;
                      } catch (e) {
                        if (
                          e instanceof TypeError ||
                          e?.plasmicType === "PlasmicUndefinedDataError"
                        ) {
                          return undefined;
                        }
                        throw e;
                      }
                    })(),
                    expertiseTitle: generateStateValueProp($state, [
                      "profileExpertiseItem2",
                      __plasmic_idx_0,
                      "expertiseTitle"
                    ]),
                    expertiseValue: generateStateValueProp($state, [
                      "profileExpertiseItem2",
                      __plasmic_idx_0,
                      "expertiseValue"
                    ]),
                    key: currentIndex,
                    onDegreeValueChange: async (...eventArgs: any) => {
                      generateStateOnChangeProp($state, [
                        "profileExpertiseItem2",
                        __plasmic_idx_0,
                        "degreeValue"
                      ]).apply(null, eventArgs);

                      if (
                        eventArgs.length > 1 &&
                        eventArgs[1] &&
                        eventArgs[1]._plasmic_state_init_
                      ) {
                        return;
                      }

                      (async val => {
                        const $steps = {};

                        $steps["updateExpertiseArray"] = true
                          ? (() => {
                              const actionArgs = {
                                variable: {
                                  objRoot: $state,
                                  variablePath: ["expertiseArray"]
                                },
                                operation: 0,
                                value: $state.expertiseArray.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        id: currentItem.id,
                                        degree_id: val,
                                        alias_title: currentItem.alias_title,
                                        expertise_id: currentItem.expertise_id
                                      }
                                    : item
                                )
                              };
                              return (({
                                variable,
                                value,
                                startIndex,
                                deleteCount
                              }) => {
                                if (!variable) {
                                  return;
                                }
                                const { objRoot, variablePath } = variable;

                                $stateSet(objRoot, variablePath, value);
                                return value;
                              })?.apply(null, [actionArgs]);
                            })()
                          : undefined;
                        if (
                          $steps["updateExpertiseArray"] != null &&
                          typeof $steps["updateExpertiseArray"] === "object" &&
                          typeof $steps["updateExpertiseArray"].then ===
                            "function"
                        ) {
                          $steps["updateExpertiseArray"] =
                            await $steps["updateExpertiseArray"];
                        }
                      }).apply(null, eventArgs);
                    },
                    onDeleteClick: async id => {
                      const $steps = {};

                      $steps["updateExpertiseArray"] = true
                        ? (() => {
                            const actionArgs = {
                              variable: {
                                objRoot: $state,
                                variablePath: ["expertiseArray"]
                              },
                              operation: 0,
                              value: $state.expertiseArray.filter(
                                item => item.id !== currentItem.id
                              )
                            };
                            return (({
                              variable,
                              value,
                              startIndex,
                              deleteCount
                            }) => {
                              if (!variable) {
                                return;
                              }
                              const { objRoot, variablePath } = variable;

                              $stateSet(objRoot, variablePath, value);
                              return value;
                            })?.apply(null, [actionArgs]);
                          })()
                        : undefined;
                      if (
                        $steps["updateExpertiseArray"] != null &&
                        typeof $steps["updateExpertiseArray"] === "object" &&
                        typeof $steps["updateExpertiseArray"].then ===
                          "function"
                      ) {
                        $steps["updateExpertiseArray"] =
                          await $steps["updateExpertiseArray"];
                      }
                    },
                    onExpertiseTitleChange: async (...eventArgs: any) => {
                      generateStateOnChangeProp($state, [
                        "profileExpertiseItem2",
                        __plasmic_idx_0,
                        "expertiseTitle"
                      ]).apply(null, eventArgs);

                      if (
                        eventArgs.length > 1 &&
                        eventArgs[1] &&
                        eventArgs[1]._plasmic_state_init_
                      ) {
                        return;
                      }

                      (async val => {
                        const $steps = {};

                        $steps["updateExpertiseArray"] = true
                          ? (() => {
                              const actionArgs = {
                                variable: {
                                  objRoot: $state,
                                  variablePath: ["expertiseArray"]
                                },
                                operation: 0,
                                value: $state.expertiseArray.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        id: currentItem.id,
                                        degree_id: currentItem.degree_id,
                                        alias_title: val,
                                        expertise_id: currentItem.expertise_id
                                      }
                                    : item
                                )
                              };
                              return (({
                                variable,
                                value,
                                startIndex,
                                deleteCount
                              }) => {
                                if (!variable) {
                                  return;
                                }
                                const { objRoot, variablePath } = variable;

                                $stateSet(objRoot, variablePath, value);
                                return value;
                              })?.apply(null, [actionArgs]);
                            })()
                          : undefined;
                        if (
                          $steps["updateExpertiseArray"] != null &&
                          typeof $steps["updateExpertiseArray"] === "object" &&
                          typeof $steps["updateExpertiseArray"].then ===
                            "function"
                        ) {
                          $steps["updateExpertiseArray"] =
                            await $steps["updateExpertiseArray"];
                        }
                      }).apply(null, eventArgs);
                    },
                    onExpertiseValueChange: async (...eventArgs: any) => {
                      generateStateOnChangeProp($state, [
                        "profileExpertiseItem2",
                        __plasmic_idx_0,
                        "expertiseValue"
                      ]).apply(null, eventArgs);

                      if (
                        eventArgs.length > 1 &&
                        eventArgs[1] &&
                        eventArgs[1]._plasmic_state_init_
                      ) {
                        return;
                      }

                      (async val => {
                        const $steps = {};

                        $steps["updateExpertiseArray"] = true
                          ? (() => {
                              const actionArgs = {
                                variable: {
                                  objRoot: $state,
                                  variablePath: ["expertiseArray"]
                                },
                                operation: 0,
                                value: $state.expertiseArray.map(item =>
                                  item.id == currentItem.id
                                    ? {
                                        id: currentItem.id,
                                        degree_id: currentItem.degree_id,
                                        alias_title: currentItem.alias_title,
                                        expertise_id: val
                                      }
                                    : item
                                )
                              };
                              return (({
                                variable,
                                value,
                                startIndex,
                                deleteCount
                              }) => {
                                if (!variable) {
                                  return;
                                }
                                const { objRoot, variablePath } = variable;

                                $stateSet(objRoot, variablePath, value);
                                return value;
                              })?.apply(null, [actionArgs]);
                            })()
                          : undefined;
                        if (
                          $steps["updateExpertiseArray"] != null &&
                          typeof $steps["updateExpertiseArray"] === "object" &&
                          typeof $steps["updateExpertiseArray"].then ===
                            "function"
                        ) {
                          $steps["updateExpertiseArray"] =
                            await $steps["updateExpertiseArray"];
                        }
                      }).apply(null, eventArgs);
                    }
                  };

                  initializePlasmicStates(
                    $state,
                    [
                      {
                        name: "profileExpertiseItem2[].degreeValue",
                        initFunc: ({ $props, $state, $queries }) =>
                          currentItem.academic_degree_id
                      },
                      {
                        name: "profileExpertiseItem2[].expertiseValue",
                        initFunc: ({ $props, $state, $queries }) =>
                          (() => {
                            try {
                              return currentItem.speciality_id;
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })()
                      },
                      {
                        name: "profileExpertiseItem2[].expertiseTitle",
                        initFunc: ({ $props, $state, $queries }) =>
                          (() => {
                            try {
                              return currentItem.alias;
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })()
                      }
                    ],
                    [__plasmic_idx_0]
                  );
                  return (
                    <ProfileExpertiseItem
                      data-plasmic-name={"profileExpertiseItem2"}
                      data-plasmic-override={overrides.profileExpertiseItem2}
                      {...child$Props}
                    />
                  );
                })();
              })}
            </div>
            <div className={classNames(projectcss.all, sty.freeBox___3Ija4)}>
              <Button
                children2={
                  "\u0630\u062e\u06cc\u0631\u0647 \u06a9\u0631\u062f\u0646 \u062a\u063a\u06cc\u06cc\u0631\u0627\u062a"
                }
                className={classNames("__wab_instance", sty.button__wTEc8)}
                loading={(() => {
                  try {
                    return $state.isLoadingSave;
                  } catch (e) {
                    if (
                      e instanceof TypeError ||
                      e?.plasmicType === "PlasmicUndefinedDataError"
                    ) {
                      return [];
                    }
                    throw e;
                  }
                })()}
                onClick={async event => {
                  const $steps = {};

                  $steps["updateIsLoadingSave"] = true
                    ? (() => {
                        const actionArgs = {
                          variable: {
                            objRoot: $state,
                            variablePath: ["isLoadingSave"]
                          },
                          operation: 0,
                          value: true
                        };
                        return (({
                          variable,
                          value,
                          startIndex,
                          deleteCount
                        }) => {
                          if (!variable) {
                            return;
                          }
                          const { objRoot, variablePath } = variable;

                          $stateSet(objRoot, variablePath, value);
                          return value;
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["updateIsLoadingSave"] != null &&
                    typeof $steps["updateIsLoadingSave"] === "object" &&
                    typeof $steps["updateIsLoadingSave"].then === "function"
                  ) {
                    $steps["updateIsLoadingSave"] =
                      await $steps["updateIsLoadingSave"];
                  }

                  $steps["runCode"] = true
                    ? (() => {
                        const actionArgs = {
                          customFunction: async () => {
                            return (async () => {
                              const newExperties = $state.expertiseArray || [];
                              const oldExperties = $state.changedExperies || [];
                              async function postItems() {
                                try {
                                  const newRequests = newExperties.map(
                                    ({ id, ...data }) =>
                                      $$.axios.post(
                                        "https://api.paziresh24.com/V1/doctor/expertises",
                                        data,
                                        {
                                          ...$ctx.Fragment.apiConfig,
                                          ...$ctx.Fragment.previewApiConfig
                                        }
                                      )
                                  );
                                  const oldRequests = oldExperties.map(
                                    ({ id, ...data }) =>
                                      $$.axios.put(
                                        `https://api.paziresh24.com/V1/doctor/expertises/${id}`,
                                        data,
                                        {
                                          ...$ctx.Fragment.apiConfig,
                                          ...$ctx.Fragment.previewApiConfig
                                        }
                                      )
                                  );
                                  const responses = await Promise.all([
                                    ...newRequests,
                                    ...oldRequests
                                  ]);
                                  console.log(
                                    "All items posted successfully:",
                                    responses
                                  );
                                  if (responses?.length > 0) {
                                    return true;
                                  }
                                } catch (error) {
                                  console.error("Error posting items:", error);
                                  return false;
                                }
                              }
                              return postItems();
                            })();
                          }
                        };
                        return (({ customFunction }) => {
                          return customFunction();
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["runCode"] != null &&
                    typeof $steps["runCode"] === "object" &&
                    typeof $steps["runCode"].then === "function"
                  ) {
                    $steps["runCode"] = await $steps["runCode"];
                  }

                  $steps["updateIsLoadingSave2"] = true
                    ? (() => {
                        const actionArgs = {
                          variable: {
                            objRoot: $state,
                            variablePath: ["isLoadingSave"]
                          },
                          operation: 0,
                          value: false
                        };
                        return (({
                          variable,
                          value,
                          startIndex,
                          deleteCount
                        }) => {
                          if (!variable) {
                            return;
                          }
                          const { objRoot, variablePath } = variable;

                          $stateSet(objRoot, variablePath, value);
                          return value;
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["updateIsLoadingSave2"] != null &&
                    typeof $steps["updateIsLoadingSave2"] === "object" &&
                    typeof $steps["updateIsLoadingSave2"].then === "function"
                  ) {
                    $steps["updateIsLoadingSave2"] =
                      await $steps["updateIsLoadingSave2"];
                  }

                  $steps["showToast"] = (() => {
                    return $steps.runCode === true;
                  })()
                    ? (() => {
                        const actionArgs = {
                          args: [
                            undefined,
                            "\u062f\u0631\u062e\u0648\u0627\u0633\u062a \u0634\u0645\u0627 \u0628\u0627 \u0645\u0648\u0641\u0642\u06cc\u062a \u067e\u0631\u062f\u0627\u0632\u0634 \u0634\u062f"
                          ]
                        };
                        return $globalActions["Fragment.showToast"]?.apply(
                          null,
                          [...actionArgs.args]
                        );
                      })()
                    : undefined;
                  if (
                    $steps["showToast"] != null &&
                    typeof $steps["showToast"] === "object" &&
                    typeof $steps["showToast"].then === "function"
                  ) {
                    $steps["showToast"] = await $steps["showToast"];
                  }

                  $steps["showToastError"] = (() => {
                    return $steps.runCode === false;
                  })()
                    ? (() => {
                        const actionArgs = {
                          args: [
                            "error",
                            "\u062e\u0637\u0627\u06cc\u06cc \u0631\u062e \u062f\u0627\u062f"
                          ]
                        };
                        return $globalActions["Fragment.showToast"]?.apply(
                          null,
                          [...actionArgs.args]
                        );
                      })()
                    : undefined;
                  if (
                    $steps["showToastError"] != null &&
                    typeof $steps["showToastError"] === "object" &&
                    typeof $steps["showToastError"].then === "function"
                  ) {
                    $steps["showToastError"] = await $steps["showToastError"];
                  }

                  $steps["sendEvent"] = true
                    ? (() => {
                        const actionArgs = {
                          args: [
                            (() => {
                              try {
                                return {
                                  event_group: "edit-profile",
                                  data: {
                                    pagePath: window.location.href,
                                    username: $state.authApi.data.data.username,
                                    userid: $state.authApi.data.data.id,
                                    userinfoid: $state.authApi.data.data.user_id
                                  },
                                  event_type: "save-changes-expertise"
                                };
                              } catch (e) {
                                if (
                                  e instanceof TypeError ||
                                  e?.plasmicType === "PlasmicUndefinedDataError"
                                ) {
                                  return undefined;
                                }
                                throw e;
                              }
                            })()
                          ]
                        };
                        return $globalActions["Splunk.sendLog"]?.apply(null, [
                          ...actionArgs.args
                        ]);
                      })()
                    : undefined;
                  if (
                    $steps["sendEvent"] != null &&
                    typeof $steps["sendEvent"] === "object" &&
                    typeof $steps["sendEvent"].then === "function"
                  ) {
                    $steps["sendEvent"] = await $steps["sendEvent"];
                  }

                  $steps["cleanUp"] = true
                    ? (() => {
                        const actionArgs = {
                          args: [
                            "POST",
                            "https://apigw.paziresh24.com/v1/n8n-nelson/webhook/Cleanup",
                            undefined,
                            (() => {
                              try {
                                return {
                                  doctor_id: $state.authApi.data?.data?.user_id,
                                  server_id: 1
                                };
                              } catch (e) {
                                if (
                                  e instanceof TypeError ||
                                  e?.plasmicType === "PlasmicUndefinedDataError"
                                ) {
                                  return undefined;
                                }
                                throw e;
                              }
                            })()
                          ]
                        };
                        return $globalActions["Fragment.apiRequest"]?.apply(
                          null,
                          [...actionArgs.args]
                        );
                      })()
                    : undefined;
                  if (
                    $steps["cleanUp"] != null &&
                    typeof $steps["cleanUp"] === "object" &&
                    typeof $steps["cleanUp"].then === "function"
                  ) {
                    $steps["cleanUp"] = await $steps["cleanUp"];
                  }

                  $steps["runActionOnExpertisesApi"] = true
                    ? (() => {
                        const actionArgs = {
                          tplRef: "expertisesApi",
                          action: "refresh"
                        };
                        return (({ tplRef, action, args }) => {
                          return $refs?.[tplRef]?.[action]?.(...(args ?? []));
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["runActionOnExpertisesApi"] != null &&
                    typeof $steps["runActionOnExpertisesApi"] === "object" &&
                    typeof $steps["runActionOnExpertisesApi"].then ===
                      "function"
                  ) {
                    $steps["runActionOnExpertisesApi"] =
                      await $steps["runActionOnExpertisesApi"];
                  }
                }}
              />

              <Button
                children2={
                  <Icon2Icon
                    className={classNames(projectcss.all, sty.svg__fAu9)}
                    role={"img"}
                  />
                }
                className={classNames("__wab_instance", sty.button__k32Sc)}
                onClick={async event => {
                  const $steps = {};

                  $steps["updateExpertiseArray"] = true
                    ? (() => {
                        const actionArgs = {
                          variable: {
                            objRoot: $state,
                            variablePath: ["expertiseArray"]
                          },
                          operation: 5,
                          value: {
                            id: $$.uuid.v4(),
                            academic_degree_id: undefined,
                            alias: "",
                            speciality_id: undefined
                          }
                        };
                        return (({
                          variable,
                          value,
                          startIndex,
                          deleteCount
                        }) => {
                          if (!variable) {
                            return;
                          }
                          const { objRoot, variablePath } = variable;

                          const arr = $stateGet(objRoot, variablePath);
                          arr.push(value);
                          return arr;
                        })?.apply(null, [actionArgs]);
                      })()
                    : undefined;
                  if (
                    $steps["updateExpertiseArray"] != null &&
                    typeof $steps["updateExpertiseArray"] === "object" &&
                    typeof $steps["updateExpertiseArray"].then === "function"
                  ) {
                    $steps["updateExpertiseArray"] =
                      await $steps["updateExpertiseArray"];
                  }

                  $steps["sendEvent"] = true
                    ? (() => {
                        const actionArgs = {
                          args: [
                            (() => {
                              try {
                                return {
                                  event_group: "edit-profile",
                                  data: {
                                    pagePath: window.location.href,
                                    username: $state.authApi.data.data.username,
                                    userid: $state.authApi.data.data.id,
                                    userinfoid: $state.authApi.data.data.user_id
                                  },
                                  event_type: "add-expertise"
                                };
                              } catch (e) {
                                if (
                                  e instanceof TypeError ||
                                  e?.plasmicType === "PlasmicUndefinedDataError"
                                ) {
                                  return undefined;
                                }
                                throw e;
                              }
                            })()
                          ]
                        };
                        return $globalActions["Splunk.sendLog"]?.apply(null, [
                          ...actionArgs.args
                        ]);
                      })()
                    : undefined;
                  if (
                    $steps["sendEvent"] != null &&
                    typeof $steps["sendEvent"] === "object" &&
                    typeof $steps["sendEvent"].then === "function"
                  ) {
                    $steps["sendEvent"] = await $steps["sendEvent"];
                  }
                }}
                outline={true}
              />
            </div>
          </div>
        </ApiRequest>
      </ApiRequest>
      <Dialog
        data-plasmic-name={"dialog"}
        data-plasmic-override={overrides.dialog}
        body={
          <div className={classNames(projectcss.all, sty.freeBox__zSbRq)}>
            <Button
              children2={
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__a5AHs
                  )}
                >
                  {"\u0628\u0644\u0647 \u0648 \u062d\u0630\u0641"}
                </div>
              }
              className={classNames("__wab_instance", sty.button__tt0Tp)}
              color={"red"}
              loading={(() => {
                try {
                  return $state.isLoadingDelete;
                } catch (e) {
                  if (
                    e instanceof TypeError ||
                    e?.plasmicType === "PlasmicUndefinedDataError"
                  ) {
                    return [];
                  }
                  throw e;
                }
              })()}
              onClick={async event => {
                const $steps = {};

                $steps["updateIsLoadingDelete"] = true
                  ? (() => {
                      const actionArgs = {
                        variable: {
                          objRoot: $state,
                          variablePath: ["isLoadingDelete"]
                        },
                        operation: 0,
                        value: true
                      };
                      return (({
                        variable,
                        value,
                        startIndex,
                        deleteCount
                      }) => {
                        if (!variable) {
                          return;
                        }
                        const { objRoot, variablePath } = variable;

                        $stateSet(objRoot, variablePath, value);
                        return value;
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["updateIsLoadingDelete"] != null &&
                  typeof $steps["updateIsLoadingDelete"] === "object" &&
                  typeof $steps["updateIsLoadingDelete"].then === "function"
                ) {
                  $steps["updateIsLoadingDelete"] =
                    await $steps["updateIsLoadingDelete"];
                }

                $steps["deleteExpertisApi"] = true
                  ? (() => {
                      const actionArgs = {
                        args: [
                          "DELETE",
                          (() => {
                            try {
                              return `https://api.paziresh24.com/V1/doctor/expertises/${$state.selectedId}`;
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })(),
                          undefined,
                          (() => {
                            try {
                              return {
                                ...$ctx.Fragment.apiConfig,
                                ...$ctx.Fragment.previewApiConfig
                              };
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })(),
                          (() => {
                            try {
                              return {
                                ...$ctx.Fragment.apiConfig,
                                ...$ctx.Fragment.previewApiConfig
                              };
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })()
                        ]
                      };
                      return $globalActions["Fragment.apiRequest"]?.apply(
                        null,
                        [...actionArgs.args]
                      );
                    })()
                  : undefined;
                if (
                  $steps["deleteExpertisApi"] != null &&
                  typeof $steps["deleteExpertisApi"] === "object" &&
                  typeof $steps["deleteExpertisApi"].then === "function"
                ) {
                  $steps["deleteExpertisApi"] =
                    await $steps["deleteExpertisApi"];
                }

                $steps["updateIsLoadingDelete2"] = true
                  ? (() => {
                      const actionArgs = {
                        variable: {
                          objRoot: $state,
                          variablePath: ["isLoadingDelete"]
                        },
                        operation: 0,
                        value: false
                      };
                      return (({
                        variable,
                        value,
                        startIndex,
                        deleteCount
                      }) => {
                        if (!variable) {
                          return;
                        }
                        const { objRoot, variablePath } = variable;

                        $stateSet(objRoot, variablePath, value);
                        return value;
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["updateIsLoadingDelete2"] != null &&
                  typeof $steps["updateIsLoadingDelete2"] === "object" &&
                  typeof $steps["updateIsLoadingDelete2"].then === "function"
                ) {
                  $steps["updateIsLoadingDelete2"] =
                    await $steps["updateIsLoadingDelete2"];
                }

                $steps["toast"] = !!$steps.deleteExpertisApi.data.message
                  ? (() => {
                      const actionArgs = {
                        args: [
                          (() => {
                            try {
                              return $steps.deleteExpertisApi.status == 200
                                ? "success"
                                : "error";
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })(),
                          (() => {
                            try {
                              return $steps.deleteExpertisApi.data.message;
                            } catch (e) {
                              if (
                                e instanceof TypeError ||
                                e?.plasmicType === "PlasmicUndefinedDataError"
                              ) {
                                return undefined;
                              }
                              throw e;
                            }
                          })()
                        ]
                      };
                      return $globalActions["Fragment.showToast"]?.apply(null, [
                        ...actionArgs.args
                      ]);
                    })()
                  : undefined;
                if (
                  $steps["toast"] != null &&
                  typeof $steps["toast"] === "object" &&
                  typeof $steps["toast"].then === "function"
                ) {
                  $steps["toast"] = await $steps["toast"];
                }

                $steps["updateDialogOpen"] = true
                  ? (() => {
                      const actionArgs = {
                        variable: {
                          objRoot: $state,
                          variablePath: ["dialog", "open"]
                        },
                        operation: 4
                      };
                      return (({
                        variable,
                        value,
                        startIndex,
                        deleteCount
                      }) => {
                        if (!variable) {
                          return;
                        }
                        const { objRoot, variablePath } = variable;

                        const oldValue = $stateGet(objRoot, variablePath);
                        $stateSet(objRoot, variablePath, !oldValue);
                        return !oldValue;
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["updateDialogOpen"] != null &&
                  typeof $steps["updateDialogOpen"] === "object" &&
                  typeof $steps["updateDialogOpen"].then === "function"
                ) {
                  $steps["updateDialogOpen"] = await $steps["updateDialogOpen"];
                }

                $steps["runActionOnProviderApi"] = true
                  ? (() => {
                      const actionArgs = {
                        tplRef: "expertisesApi",
                        action: "refresh"
                      };
                      return (({ tplRef, action, args }) => {
                        return $refs?.[tplRef]?.[action]?.(...(args ?? []));
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["runActionOnProviderApi"] != null &&
                  typeof $steps["runActionOnProviderApi"] === "object" &&
                  typeof $steps["runActionOnProviderApi"].then === "function"
                ) {
                  $steps["runActionOnProviderApi"] =
                    await $steps["runActionOnProviderApi"];
                }
              }}
            />

            <Button
              children2={
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__nu3Ag
                  )}
                >
                  {"\u0644\u063a\u0648"}
                </div>
              }
              className={classNames("__wab_instance", sty.button__lcR8T)}
              color={"softSand"}
              onClick={async event => {
                const $steps = {};

                $steps["updateDialogOpen"] = true
                  ? (() => {
                      const actionArgs = {
                        variable: {
                          objRoot: $state,
                          variablePath: ["dialog", "open"]
                        },
                        operation: 4
                      };
                      return (({
                        variable,
                        value,
                        startIndex,
                        deleteCount
                      }) => {
                        if (!variable) {
                          return;
                        }
                        const { objRoot, variablePath } = variable;

                        const oldValue = $stateGet(objRoot, variablePath);
                        $stateSet(objRoot, variablePath, !oldValue);
                        return !oldValue;
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["updateDialogOpen"] != null &&
                  typeof $steps["updateDialogOpen"] === "object" &&
                  typeof $steps["updateDialogOpen"].then === "function"
                ) {
                  $steps["updateDialogOpen"] = await $steps["updateDialogOpen"];
                }

                $steps["updateSelectedId"] = true
                  ? (() => {
                      const actionArgs = {
                        variable: {
                          objRoot: $state,
                          variablePath: ["selectedId"]
                        },
                        operation: 1
                      };
                      return (({
                        variable,
                        value,
                        startIndex,
                        deleteCount
                      }) => {
                        if (!variable) {
                          return;
                        }
                        const { objRoot, variablePath } = variable;

                        $stateSet(objRoot, variablePath, undefined);
                        return undefined;
                      })?.apply(null, [actionArgs]);
                    })()
                  : undefined;
                if (
                  $steps["updateSelectedId"] != null &&
                  typeof $steps["updateSelectedId"] === "object" &&
                  typeof $steps["updateSelectedId"].then === "function"
                ) {
                  $steps["updateSelectedId"] = await $steps["updateSelectedId"];
                }
              }}
            />
          </div>
        }
        className={classNames("__wab_instance", sty.dialog)}
        noTrigger={true}
        onOpenChange={async (...eventArgs: any) => {
          generateStateOnChangeProp($state, ["dialog", "open"]).apply(
            null,
            eventArgs
          );

          if (
            eventArgs.length > 1 &&
            eventArgs[1] &&
            eventArgs[1]._plasmic_state_init_
          ) {
            return;
          }

          (async val => {
            const $steps = {};

            $steps["updateSelectedId"] = !val
              ? (() => {
                  const actionArgs = {
                    variable: {
                      objRoot: $state,
                      variablePath: ["selectedId"]
                    },
                    operation: 1
                  };
                  return (({ variable, value, startIndex, deleteCount }) => {
                    if (!variable) {
                      return;
                    }
                    const { objRoot, variablePath } = variable;

                    $stateSet(objRoot, variablePath, undefined);
                    return undefined;
                  })?.apply(null, [actionArgs]);
                })()
              : undefined;
            if (
              $steps["updateSelectedId"] != null &&
              typeof $steps["updateSelectedId"] === "object" &&
              typeof $steps["updateSelectedId"].then === "function"
            ) {
              $steps["updateSelectedId"] = await $steps["updateSelectedId"];
            }
          }).apply(null, eventArgs);
        }}
        open={generateStateValueProp($state, ["dialog", "open"])}
        title={
          "\u0622\u06cc\u0627 \u0627\u0632 \u062d\u0630\u0641 \u062a\u062e\u0635\u0635 \u0645\u0637\u0645\u0626\u0646 \u0647\u0633\u062a\u06cc\u062f\u061f"
        }
        trigger={null}
      />
    </div>
  ) as React.ReactElement | null;
}

const PlasmicDescendants = {
  root: [
    "root",
    "authApi",
    "expertisesApi",
    "profileExpertiseItem",
    "h6",
    "profileExpertiseItem2",
    "dialog"
  ],
  authApi: [
    "authApi",
    "expertisesApi",
    "profileExpertiseItem",
    "h6",
    "profileExpertiseItem2"
  ],
  expertisesApi: [
    "expertisesApi",
    "profileExpertiseItem",
    "h6",
    "profileExpertiseItem2"
  ],
  profileExpertiseItem: ["profileExpertiseItem"],
  h6: ["h6"],
  profileExpertiseItem2: ["profileExpertiseItem2"],
  dialog: ["dialog"]
} as const;
type NodeNameType = keyof typeof PlasmicDescendants;
type DescendantsType<T extends NodeNameType> =
  (typeof PlasmicDescendants)[T][number];
type NodeDefaultElementType = {
  root: "div";
  authApi: typeof ApiRequest;
  expertisesApi: typeof ApiRequest;
  profileExpertiseItem: typeof ProfileExpertiseItem;
  h6: "h6";
  profileExpertiseItem2: typeof ProfileExpertiseItem;
  dialog: typeof Dialog;
};

type ReservedPropsType = "variants" | "args" | "overrides";
type NodeOverridesType<T extends NodeNameType> = Pick<
  PlasmicProfileExperties__OverridesType,
  DescendantsType<T>
>;
type NodeComponentProps<T extends NodeNameType> =
  // Explicitly specify variants, args, and overrides as objects
  {
    variants?: PlasmicProfileExperties__VariantsArgs;
    args?: PlasmicProfileExperties__ArgsType;
    overrides?: NodeOverridesType<T>;
  } &
    // Specify variants directly as props
    Omit<PlasmicProfileExperties__VariantsArgs, ReservedPropsType> &
    // Specify args directly as props
    Omit<PlasmicProfileExperties__ArgsType, ReservedPropsType> &
    // Specify overrides for each element directly as props
    Omit<
      NodeOverridesType<T>,
      ReservedPropsType | VariantPropType | ArgPropType
    > &
    // Specify props for the root element
    Omit<
      Partial<React.ComponentProps<NodeDefaultElementType[T]>>,
      ReservedPropsType | VariantPropType | ArgPropType | DescendantsType<T>
    >;

function makeNodeComponent<NodeName extends NodeNameType>(nodeName: NodeName) {
  type PropsType = NodeComponentProps<NodeName> & { key?: React.Key };
  const func = function <T extends PropsType>(
    props: T & StrictProps<T, PropsType>
  ) {
    const { variants, args, overrides } = React.useMemo(
      () =>
        deriveRenderOpts(props, {
          name: nodeName,
          descendantNames: PlasmicDescendants[nodeName],
          internalArgPropNames: PlasmicProfileExperties__ArgProps,
          internalVariantPropNames: PlasmicProfileExperties__VariantProps
        }),
      [props, nodeName]
    );
    return PlasmicProfileExperties__RenderFunc({
      variants,
      args,
      overrides,
      forNode: nodeName
    });
  };
  if (nodeName === "root") {
    func.displayName = "PlasmicProfileExperties";
  } else {
    func.displayName = `PlasmicProfileExperties.${nodeName}`;
  }
  return func;
}

export const PlasmicProfileExperties = Object.assign(
  // Top-level PlasmicProfileExperties renders the root element
  makeNodeComponent("root"),
  {
    // Helper components rendering sub-elements
    authApi: makeNodeComponent("authApi"),
    expertisesApi: makeNodeComponent("expertisesApi"),
    profileExpertiseItem: makeNodeComponent("profileExpertiseItem"),
    h6: makeNodeComponent("h6"),
    profileExpertiseItem2: makeNodeComponent("profileExpertiseItem2"),
    dialog: makeNodeComponent("dialog"),

    // Metadata about props expected for PlasmicProfileExperties
    internalVariantProps: PlasmicProfileExperties__VariantProps,
    internalArgProps: PlasmicProfileExperties__ArgProps
  }
);

export default PlasmicProfileExperties;
/* prettier-ignore-end */
